 ...
 * пересобирает потребность цеха целиком (а если полный пересчёт законно
 * отбит строками в работе у закупщика — синхронизирует строки нанесений
 * точечно и ставит на заказ отметку `needsStaleAt` с причиной; аудит
 * движка расчёта 13.09.2026, G10-1), после — синхронизирует
