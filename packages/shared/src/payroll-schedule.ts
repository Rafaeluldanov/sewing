...
export const PAYROLL_CUTOFF_BASIS_HINTS: Record<PayrollCutoffBasis, string> = {
  ...
  PASSPORT_PACKED:
    'Платим по подтверждённой работе, не дожидаясь закрытия всего заказа: сдельщина подтверждается в момент закрытия коробки. От «дня выполнения работы» отличается только в документе задним числом: не войдёт то, что подтвердили уже после дня начисления.',
  WORK_DATE:
    'Входит вся подтверждённая сдельщина с датой работы до дня начисления, независимо от статуса заказа. Неупакованное не войдёт и здесь: подтверждение ставится закрытием коробки, и отсечка на это не влияет.',
};
...
/**
 * Флаг охвата в query-строке. Приходит строкой (`?appliesToSewing=1`),
 * поэтому `z.coerce.boolean()` не годится: он считает истиной ЛЮБУЮ
 * непустую строку, и `=0` молча включал бы правило.
 */
const QueryFlag = z
  .enum(['0', '1', 'true', 'false'])
...
/**
 * Query предпросмотра. Кроме даты принимает НЕСОХРАНЁННОЕ правило:
 * экран «Правила начисления» пересчитывает суммы прямо по
 * переключателю, до нажатия «Сохранить». Без этого предпросмотр всегда
 * считал по строке из БД, и переключение варианта не двигало сумму —
 * менеджер видел последствие только после сохранения, то есть уже
 * применив правило к деньгам.
 *
 * Поля независимы: переданное перекрывает сохранённое, остальное
 * берётся из настройки.
 */
export const PayrollAccrualPreviewQuerySchema = z.object({
  accrualDate: RealDateOnly.optional(),
  cutoffBasis: PayrollCutoffBasisSchema.optional(),
  appliesToSewing: QueryFlag,
  appliesToCutting: QueryFlag,
});
...
export interface PayrollAccrualPreviewDto {
  ...
  cutoffBasis: PayrollCutoffBasis;
  /**
   * Охват, с которым посчитан ЭТОТ предпросмотр (не обязательно
   * сохранённый — см. `PayrollAccrualPreviewQuerySchema`). Нужен
   * экрану: при обоих `false` правило отсечки не применяется ни к
   * чему, и сумма одинакова для всех трёх вариантов.
   */
  appliesToSewing: boolean;
  appliesToCutting: boolean;
