...
export interface ProductionMaterialLineDto {
  ...
  /**
   * Сумма строки, ₽. `null` — цена есть, но в рублях её не выразить: котировка/закупка в USD, а
   * курса в активной смете нет (код `MATERIAL_PRICE_USD_NO_RATE`), либо валюта, которую движок не
   * переводит вовсе — EUR, CNY, … (код `MATERIAL_PRICE_CURRENCY_UNSUPPORTED`, ревью E1-7). Это не
   * 0 ₽ — в сумму документа такая строка не входит, и показывать её надо прочерком, а не нулём
   * (аудит движка расчёта 13.09.2026, E1-7).
   */
