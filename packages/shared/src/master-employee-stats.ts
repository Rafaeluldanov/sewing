...
export interface MasterActiveShiftDto {
   ...
   * это только предупреждение мастеру (решение №12 аудита 13.09.2026 —
   * «жёсткая граница сменой или предупреждение» — владельцем не принято).
   */
  hasActiveRecut: boolean;
  /**
   * Активный подкрой вышел за границы (Аудит движка расчёта 13.09.2026,
   * G4-3, ревью): начат в уже ЗАКРЫТОЙ смене (пережил её) либо тикает
   * дольше предела `shiftMaxDurationHours` / 16 ч — доплата при
   * завершении будет обрезана предохранителем. Мастеру стоит напомнить
   * раскройщику остановить таймер. `false`, если `hasActiveRecut = false`.
   */
  activeRecutOverLimit: boolean;
}
