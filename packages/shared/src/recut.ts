...
export interface RecutSessionDto {
  ...
  amount: number | null;
  /**
   * Предупреждение (Аудит движка расчёта 13.09.2026, G4-3, ревью):
   * подкрой длиннее своей смены — смена, в которой он начат, уже
   * закрыта, а таймер идёт (`ACTIVE`) или был остановлен после её конца
   * (`DONE`). Концом смены подкрой НЕ режется и закрытием смены НЕ
   * завершается — «жёсткая граница или предупреждение мастеру» (решение
   * №12) владельцем не принято; пока — только этот флаг и пометка в
   * строке `RECUT` ведомости.
   */
  longerThanShift: boolean;
  /**
   * Предохранитель сработал: `DONE` — `workedSeconds` меньше фактического
   * `endedAt − startedAt` (предел `shiftMaxDurationHours` или 16 ч, тот же,
   * что у часов смены, K7); `ACTIVE` — таймер уже тикает дольше предела,
   * доплата при завершении будет обрезана.
   */
