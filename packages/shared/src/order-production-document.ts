...
export const PRODUCTION_DOC_WARNING_LABELS: Record<string, string> = {
  ...
  // Решение владельца 14.09.2026: факт окладных операций = норма × объём.
  SALARY_NORM_MISSING:
    'У окладной операции не задана норма времени — её факт по заказу учтён в 0 ₽ (заполните норму в справочнике операций)',
};
...
export interface OrderProductionOperationRowDto {
  ...
  /**
   * Штук выполнено — Σ `OperationEntry.qty` (не CANCELLED/REVERSED) плюс
   * изделия паспортов, по которым окладник закрыл операцию (по одному разу
   * на паспорт × операцию).
   */
  factQty: number;
  /**
   * Фактическая стоимость, ₽ — Σ `OperationEntry.amount` (сдельная) +
   * разнесённый оклад по факту выполненных работ (`factSalaryRub`).
   */
  factRub: string;
  /** Из них подтверждено (APPROVED сдельная + весь оклад), ₽. */
  factApprovedRub: string;
  /**
   * Из `factRub` — оклад по факту выполненных работ (Decimal-строка):
   * хронометраж в рамке смены либо норма времени × объём для ОТК/ВТО/
   * упаковки (решение владельца 14.09.2026). Раньше у окладных операций
   * (`pricingMode = SALARY_ONLY`) факт в документе был всегда 0 — они не
   * пишут `OperationEntry`, — при том что план по ним считался.
   */
