...
export interface ShopfloorDisplayRouteSizeRowDto {
  ...
  sizeSortOrder: number;
  /**
   * Нормализованный ключ цвета (lower-case, синонимы вида «чёрный/
   * black» сведены в каноническую форму) — совпадает с
   * `ShopfloorDisplayColorBlock.colorKey`, так UI ищет ячейку
   * матрицы по паре `(colorKey, size)`. Без явного цвета у паспорта /
   * заказа — `__unknown__` (тогда же `colorLabel = 'Без цвета'`).
   *
   * Добавление цвета в sewing-route — фикс инцидента «138 чёрного
   * S отображалось и в строке белого S»: до этого backend агрегировал
   * только `(operation, size)`, и UI клонировал одно значение во все
   * цветовые строки одного размера. Теперь каждая строка относится
   * ровно к одной паре `(color, size)`.
   */
  colorKey: string;
  /**
   * Отображаемая подпись цвета (нормированная подпись для известных
   * цветов из `SHOPFLOOR_DISPLAY_KNOWN_COLORS`, иначе — исходная
   * строка от паспорта/заказа). Совпадает с
   * `ShopfloorDisplayColorBlock.colorLabel`.
   */
  colorLabel: string;
  /**
