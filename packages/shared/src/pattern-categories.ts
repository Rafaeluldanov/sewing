...
export const PATTERN_CATEGORY_PARAMETER_GROUPS: readonly PatternCategoryParameterGroupConfig[] = [
  ...
  {
    // Наполнитель (синтепон / холлофайбер / спанбонд). По TEEON.pdf
    // синтепон вводится в «м пог.» по размерам (LINEAR_M_BY_SIZE) — как
    // полотна, — и в потребность попадает в «м пог.»; искусственный пух
    // задаётся нормой «кг на изделие» (QTY_PER_ITEM). Поэтому группа
    // допускает оба ввода; LINEAR_M_BY_SIZE — по умолчанию (синтепон —
    // основной кейс, в т.ч. для категории «жилетки на синтепоне»).
    // TEXT_ONLY оставляем для чисто описательных позиций.
    roleKey: 'FILLER',
    label: 'Наполнитель',
    // Синтепон — «м пог.» (с пересчётом через плотность), искусственный
    // пух — «кг»; «м» оставлен для рулонных позиций.
    allowedUnits: ['м пог.', 'кг', 'м'],
    defaultUnit: 'м пог.',
    allowedInputTypes: ['LINEAR_M_BY_SIZE', 'QTY_PER_ITEM', 'TEXT_ONLY'],
    defaultInputType: 'LINEAR_M_BY_SIZE',
  },
  {
    roleKey: 'INTERLINING',
