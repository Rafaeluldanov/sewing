...
export const OrderCalculationSnapshotV1Schema = z.object({
  ...
  order: z.object({
     ...
     * пересоздаются из `routeSteps`, без флага он сбрасывается в `null`,
     * и маршрут пересобирается из шаблона. ISO-строка; `nullish` — снимки
     * в БД лежат без поля (читается как «не правлен»).
     */
    routeCustomizedAt: z
      .string()
      .nullish()
   ...
   * снимки без поля: restore пересобирает маршрут из шаблона.
   */
  routeSteps: z
