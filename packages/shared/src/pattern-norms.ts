 ...
 * вызывающий оставляет норму шаблона. «Пересечение» здесь — размеры, у
 * которых есть И значение в номенклатуре, И ненулевой план: значение у
 * размера с нулевым тиражом норму не даёт (Аудит движка расчёта 13.09.2026,
 * T1-2 — прежний фолбэк «план пуст → среднее арифметическое» отдавал норму
 * размера, которого в тираже нет, и спецификация расходилась с потребностью,
 * где такой параметр даёт предупреждение и строки не даёт).
 */
export function derivePatternNormPerUnit(
  ...
  // Аудит движка расчёта 13.09.2026, T1-2: ни один размер с ненулевым
  // планом не покрыт значением — источник ничего не даёт. Фолбэк «среднее
  // арифметическое по заполненным размерам» убран: штатно нулевые строки
  // плана не пишутся, и единственный достижимый случай — норма размера,
  // которого в тираже нет; потребность в той же ситуации строки не даёт.
