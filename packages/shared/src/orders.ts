...
export const UpdateOrderSchema = z.object({
   ...
   * + своя техкарта + поразмерный план. Когда передан, backend приводит
   * `OrderVariant` / `OrderVariantSize` к присланной картине и пересобирает
   ...
   * одном движке. Аудит движка расчёта 13.09.2026, G9-1: расцветки при этом
   * НЕ пересоздаются — сопоставляются по цвету (остаток — по порядку, как
   * переименование), id живут, заменяется только поразмерный план; лишние
   * удаляются, недостающие создаются. Так правки внутри заказа (нормы,
   * значения параметров, ручные строки расцветок, строки потребности)
   * переживают сохранение формы. Форма шлёт `variants` ВМЕСТО `items`,
   * чтобы прямая запись агрегата не конфликтовала с ресинком. Разрешено в DRAFT /
 ...
 *
 * ⛔ Только ПОЛНАЯ картина, а не дельта: `variants` становятся планом заказа
 * целиком (тот же путь, что у формы правки: расцветки сопоставляются по цвету
 * и НЕ пересоздаются — правки цеха внутри расцветки переживают досылку,
 * см. `OrdersService.upsertOrderVariants`).
