 ...
 *   - окладная часть — разнесённое реальное время окладников
 *     (`PassportRealCostService`) по паспортам, ВЫПУЩЕННЫМ в периоде, на
 *     окне самого паспорта: `salaryAllocatedCostRub` входит в
 *     `totalCostRub` номенклатуры / заказа / итогов. Оклад по паспортам,
 *     не выпущенным в периоде, в номенклатуру не попадает — об этом
 *     warning с суммой (аудит движка расчёта 13.09.2026, F1-3 / F1-11;
 *     JSDoc «всегда 0» устарел с 7adac18).
 *
...
export interface ProductionCostOrderGroupDto {
  ...
  /**
   * Оклад, разнесённый на выпущенные в периоде паспорта этого заказа
   * (реальное время ОТК/ВТО/упаковки × ставка, окно паспорта). Входит в
   * `totalCostRub`.
   */
...
export interface ProductionCostTotalsDto {
  ...
  // -- Окладная часть за период (разнос реального времени окладников) --
  // Поля мини-отчёта «Себестоимость / Простой» в шапке. Оклад ВЫПУЩЕННЫХ
  // паспортов уже сидит в `salaryAllocatedCostRub` и в `totalCostRub`;
  // рабочая часть ниже считается по ВСЕМ паспортам с событиями в окне
  // (включая не выпущенные) и ПЕРЕСЕКАЕТСЯ с `salaryAllocatedCostRub` —
  // складывать её с `totalCostRub` нельзя (аудит движка расчёта
  // 13.09.2026, F1-11). Источник и формула простоя — те же, что в дневном
  // отчёте (`CostsService`):
  //   рабочая часть = Σ разнесённое_время × ставка/мин;
  //   простой       = Σ max(0, оплачено − разнесённое) × ставка/мин
  //                   по окладникам на смене в этот день (`SHIFT_DAY`-строка
  //                   у почасовика, закрытая смена у месячника; F1-1/F1-2).

  /**
   * Окладная «рабочая» часть за период: разнесённое реальное время × ставка,
   * ₽, по всем паспортам с событиями в окне. Справочное поле, в
   * `totalCostRub` не прибавляется.
   */
  salaryWorkingCostRub: string;
  /** Σ разнесённых (учтённых) минут окладников за период. */
  salaryWorkingMinutes: number;
  /** Простой по окладной части, ₽. */
  idleSalaryCostRub: string;
  /** Σ минут простоя окладников за период. */
...
export interface ProductionCostReportDto {
   ...
   * cost-estimate, оклад по паспортам, не выпущенным в периоде (с суммой),
   * и т. п.
   */
  warnings: string[];
}
