...
export interface ProductionCostTotalsDto {
  ...
  // Эти поля НЕ входят в `totalCostRub` (он считается по номенклатуре,
  // куда оклад в v2 не распределяется) — они нужны для мини-отчёта
  // «Себестоимость / Простой» в шапке. Источник и формула простоя —
  // те же, что в дневном отчёте (`CostsService`):
  //   рабочая часть = Σ разнесённое_время × (оклад/480);
  //   простой       = Σ max(0, 480 − разнесённое) × (оклад/480)
  //                   по окладникам с `SalaryEntry` за день (был на смене).

  /** Окладная «рабочая» часть: разнесённое реальное время × ставка, ₽. */
