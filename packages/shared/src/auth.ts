...
export interface AuthUserDto {
  ...
  role: string;
  /**
   * ЭФФЕКТИВНЫЙ набор ролей доступа: назначенные сотруднику плюс всё,
   * что они наследуют по `AppRole.inherits` (см. `expandRoleCodes` в
   * `@sewing/shared/app-roles`). Всегда содержит `role`. На него
   * опираются web-RBAC (`canSee*`) и навигация — благодаря раскрытию
   * кастомная роль «Технолог» видит те же разделы, что её донор,
   * без правки `lib/rbac.ts`. Опционально (`?`) для backward-compat —
   * backend всегда отдаёт массив.
   */
  roles?: string[];
  /**
   * НАЗНАЧЕННЫЙ набор (`Employee.roles`) — то, что отмечено галочками
   * в карточке сотрудника, БЕЗ раскрытия наследования. Нужен формам
   * («какие роли выданы») и признаку «одно рабочее окно»: у сотрудника
   * с единственной кастомной ролью раскрытый набор длиннее одного
   * элемента и «одно окно» ошибочно бы не сработало.
   */
  ...
  /**
   * Название активного участка (`AppRole.name` для `activeRole ?? role`)
   * — то, что показывает бейдж в шапке терминала. Резолвит сервер:
   * названия кастомных ролей живут в БД, а терминалам справочник
   * `/api/app-roles` не открыт (он под ADMIN/SHOP_MANAGER).
   */
  activeRoleLabel?: string;
  /**
   * Рабочий экран учётки (`AppRole.workspace`), посчитанный сервером по
   * справочнику ролей. Веб использует его вместо собственной матрицы
   * «роль → экран»: логин, корневой `/` и кнопка смены рабочего места
   * ведут сюда. Отсутствует — веб откатывается на `getPrimaryWorkspace`
   * по системным ролям.
   */
  workspace?: string;
