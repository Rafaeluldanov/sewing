...
export const ListWorkshopNeedsQuerySchema = z.object({
   ...
   *     контура вариантов: sample/legacy). Так ходят выдачи и блок
   *     «Потребность цеха» — им образец видеть НАДО;
   *   - `TIRAGE` — активный вариант И без сигнального образца. Скоуп
   *     читателей денег и материалов тиража («Сводно», «Материалы»,
   *     карточка плановой себестоимости): образец шьют отдельно, его
   *     строки — второй комплект тех же материалов на 1 изделие;
   *   - `ALL` (default) — все варианты; закупочные экраны и вкладка
   *     «Потребности» показывают их с меткой варианта.
   */
  calculationScope: z.enum(['ACTIVE', 'TIRAGE', 'ALL']).optional(),
  /**
