 ...
 * заказа (или раньше — кнопкой, на любой стадии), собирается САМ из фактов производства и
 * хранит снимок себестоимости.
 *
 * Состояний ровно два, и оба наступают без человека:
 *   `FORMING` — факты ещё идут: заказ ещё в работе, открыта коробка, начисления не подтверждены;
 *   `READY`   — лёг последний факт, себестоимость записана снимком. Без закрытого заказа
 *               не бывает: недошитый выпуск не окончателен и в ERP не уезжает.
 *
...
export interface ProductionDocumentPendingReasonDto {
  /**
   * `ORDER_OPEN` — заказ ещё не закрыт (документ заведён заранее, выпуск накапливается);
   * `ORDER_CANCELLED` — заказ отменили после того, как документ завели, выпуска не будет;
   * `OPEN_BOX` — коробка не закрыта; `PENDING_EARNINGS` — начисления не подтверждены.
   */
  code: 'ORDER_OPEN' | 'ORDER_CANCELLED' | 'OPEN_BOX' | 'PENDING_EARNINGS';
  text: string;
...
export interface ProductionDocumentListItemDto {
  ...
  perUnitRub: number;
  /**
   * Момент закрытия заказа. `null` — документ заведён кнопкой по ещё открытому заказу и
   * формируется по ходу производства; дату проставит закрытие заказа.
   */
...
export interface ProductionDocumentDto extends ProductionDocumentListItemDto {
   ...
   *
   * У документа, заведённого кнопкой по ОТКРЫТОМУ заказу, отметки нет: он не восстановлен
   * задним числом, а живёт с момента формирования (`closedAt = null`, пока заказ не закрыт).
   */
