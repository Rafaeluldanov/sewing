...
Единый демо-пароль: **`Demo12345!`** (хранится в БД только как bcrypt-hash).
В seed-е (`prisma/seed.ts`) `pinHash` всегда переустанавливается на
`Demo12345!` — поэтому после `npm run db:seed` (даже на «грязной» БД) демо-
учётки гарантированно входят через `/login` (см. ADR-0014).
