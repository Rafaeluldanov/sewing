...
const nextConfig = {
  ...
  // ---------------------------------------------------------------------------
  // Dev-сервер выбрасывает скомпилированные маршруты из памяти по таймауту, и
  // повторный заход на страницу компилируется заново (замер: холодный /admin/orders
  // — 5.4 с против 0.24 с на тёплом). Дефолты Next — 25 с и 2 страницы в буфере,
  // что для админки на ~25 маршрутов означает почти постоянную перекомпиляцию.
  // Держим час и 25 страниц: платим памятью next-server (лимит контейнера
  // mem_limit=3g в docker-compose.dev.yml), выигрываем на каждом переходе.
  // На prod-сборку не влияет — опция читается только dev-сервером.
  // ---------------------------------------------------------------------------
  onDemandEntries: {
