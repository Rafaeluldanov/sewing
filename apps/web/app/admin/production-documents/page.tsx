 ...
 *   `FORMING` — факты ещё идут (заказ ещё в работе / открыта коробка / не подтверждены
 *               начисления);
...
export default async function AdminProductionDocumentsPage({
  ...
  const columns: AdminTableColumn<ProductionDocumentListItemDto>[] = [
    ...
    },
    {
      key: 'closedAt',
      header: 'Сдан',
      // Документ по открытому заказу (заведён заранее) даты сдачи не имеет: её проставит
      // закрытие заказа. Не «—»: пустота здесь значит «заказ ещё в работе», а не «нет данных».
      render: (doc) =>
        doc.closedAt ? (
