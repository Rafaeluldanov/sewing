  ...
  return (
    <form action={formAction} className="admin-form admin-order-form">
      ...
      {/* Фича «Расцветки»: под флагом «Цвет» и расцветки заказа сабмитятся
          отсюда (видимые инпуты «Цвет» и общий выбор номенклатуры скрыты).
          ...
          <span>
            {order.status === 'CALCULATION'
              ...
              : order.status === 'CALCULATION_DONE'
                ? 'Заказ в статусе «Расчёт завершён»: до запуска производства доступны подразделение, маршрут и управленческие поля. Состав материалов, лекало и спецификацию правьте через «Вернуть на пересчёт».'
