...
export function ArchivePatternButton({ patternId, patternName, status }: Props) {
  ...
  const handleClick = () => {
    const question = archived
      ? `Вернуть номенклатуру «${patternName}» из архива? Она снова появится в активном справочнике и в подборе для заказов.`
      : `Архивировать номенклатуру «${patternName}»? Она пропадёт из активного справочника и перестанет предлагаться в заказах. Связанные заказы и паспорта не пострадают.`;
