...
export interface WorkFormState {
  error?: string;
  /**
   * Мягкое предупреждение (НЕ ошибка): действие не выполнено, но это
   * ожидаемая ситуация, о которой рабочего надо спокойно предупредить,
   * а не пугать красной ошибкой. Пример — гейт «крой не размещён в
   * ячейке» (`PASSPORT_NOT_PLACED_IN_CELL`): сначала нужно разместить
   * паспорт в ячейку. Рендерится жёлтой плашкой `role="status"`.
   */
  warning?: string;
  /**
