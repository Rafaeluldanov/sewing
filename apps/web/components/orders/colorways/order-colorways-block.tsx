...
export function OrderColorwaysBlock({
  ...
  return (
    <section className="cwl">
      ...
      {ro && (
        <div className="cwl-locked">
          ...
          <span>
            ...
            поэтому цвет, размеры и выбор номенклатуры только для просмотра:
            правка не изменит общий план заказа — чтобы менять их, верните
            заказ на пересчёт.{' '}
            {params?.editMode === 'AMENDMENT' ? (
              <>
                <strong>Состав материалов править можно</strong> — откройте
      ...
      {/* Модалка «Материалы расцветки» — компактный оверлей вместо инлайновой
      ...
      {specVariant && params && drafts[specVariant] && (
        <ModalPortal>
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-label={`Материалы расцветки — ${drafts[specVariant].color || 'расцветка'}`}
            onClick={(e) => {
          ...
          >
            <div className="qr-modal__card cwl-spec-modal__card">
              ...
              {/* readOnly НЕ пробрасываем: окно правки спецификации ШИРЕ,
                  чем у расцветок. Расцветку (цвет/размеры/номенклатуру) можно
