    ...
    return (
      <div className="needs-stale__confirm">
        <p className="needs-stale__text">
          {reviewedCount > 0 ? (
            // Аудит движка расчёта 13.09.2026, N2-2: цена и поставщик
            // переносятся на пересозданную строку (см. `buildPurchaseCarry`),
            // пропадает только статус — плашка обещала обратное. Ревью:
            // перенос — по ключу «источник + расцветка + единица», и он
            // однозначный: строка, сменившая единицу (лекало перевело
            // «м пог.» в «кг»), или неоднозначный ключ закупочный блок
            // не получают — обещать «сохранятся» безусловно нельзя.
            <>
              Строк в работе у закупщика: <b>{reviewedCount}</b>. Пересчёт
              создаст их заново — статус вернётся в «рассчитано»; цена и
              поставщик сохранятся, если единица строки не изменилась.
            </>
          ) : (
