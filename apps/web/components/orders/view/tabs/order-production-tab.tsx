  ...
  return (
    <div className="order-prod-tab">
      ...
      */}
      ...
      {/*
        «Нанесение» — рядом с расцветками, но окно правки у них РАЗНОЕ:
        расцветки замирают вместе с планом (DRAFT/CALCULATION), а
        нанесение правится на любой стадии, кроме отменённой. Причина —
        нанесение это расход заказа, а не его размерный план: клиент
        просит принт и тогда, когда тираж уже кроят. После завершения
        расчёта карточка предупреждает о правилах поздней правки, а
        backend синхронизирует потребность точечно
        (`WorkshopNeedsService.syncApplicationNeeds`). Размеры для
        адресации «на выбранные размеры» берём из планового среза заказа.
      */}
