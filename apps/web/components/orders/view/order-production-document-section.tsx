 ...
 *   заказа (или раньше — кнопкой, на любой стадии) и собирается сам из
 *   фактов производства. Состояния два, оба наступают без человека:
 *   `FORMING` (факты ещё идут — причины в `pendingReasons`, первая из них
 *   у документа по открытому заказу — сам заказ) и `READY` (лёг последний
 *   факт, себестоимость зафиксирована снимком; без закрытого заказа не
 *   бывает).
 *
 ...
 *   пересборка. Работает на любой стадии заказа (решение владельца
 *   14.09.2026): по открытому заказу документ заводится заранее и
 *   наполняется по ходу производства. Придумать выпуск ею всё равно нельзя:
 *   окончательным документ станет только с закрытием заказа, а отказы
 *   остались два — заказ отменён и заказ закрыт без упаковки. Никаких
 *   «провести» и «согласовать» здесь нет.
 *
...
interface Props {
  /**
   * Документ заказа. `null` — документа ещё нет: заказ не закрыт и заранее
   * его не формировали (это норма, а не ошибка).
   */
  doc: ProductionDocumentDto | null;
  /** Заказ. Нужен и для кнопки формирования, и для ссылок. */
  orderId: string;
  /**
   * Заказ уже ЗАКРЫТ — подсказка для текста пустого состояния: у закрытого
   * заказа документ достраивается задним числом, у открытого — заводится
   * заранее. Показ кнопки этим НЕ гейтится: человек должен уметь нажать её
   * всегда, а почему не вышло — объяснит backend («заказ отменён»,
   * «закрыт без упаковки»).
   */
  orderClosed: boolean;
  /**
        ...
        <AdminEmptyState
          ...
          hint={
            orderClosed
              ...
              : 'Сам он появится с закрытием заказа, но сформировать его можно уже сейчас: документ будет наполняться по ходу производства — упакованные паспорта, списания, начисления — и станет окончательным, когда заказ закроется.'
          }
          actions={
            canManage ? (
