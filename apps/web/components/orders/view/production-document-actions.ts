 ...
 * Работает на любой стадии заказа: по открытому документ заводится заранее и наполняется по ходу
 * производства. Придумать выпуск им нельзя: окончательным документ станет только с закрытием
 * заказа; отказы — заказ отменён и заказ закрыт без упаковки.
 *
...
export async function syncProductionDocumentAction(
  ...
  try {
    ...
    // Отказы здесь осмысленные — «заказ отменён», «закрыт без упаковки» — и человек должен
