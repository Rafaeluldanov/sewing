 ...
 * заказа»): «Производство → Паспорта → Операции → Сводно по
 * заказу → Потребности → История». Менеджер читает карточку слева
 * направо как «факт → объекты → операции → деньги → обеспечение →
 * аудит». Вкладка «План» удалена: её настроечные блоки («Цвета по
 * строкам техкарты», «Очередь выдачи кроя»), а также продукт/даты
 * переехали в «Производство» и постоянную шапку заказа. «Операции»
 * — это единый рабочий экран по
