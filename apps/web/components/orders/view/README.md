...
тематических вкладок (Производство → Паспорта → Операции
→ Сводно по заказу → Потребности → Сигнальный образец → История).
Вкладка «План» удалена: её настроечные блоки переехали в
«Производство», продукт/артикул/даты — в шапку заказа. Каждая зона отвечает
...
- продукт / лекало / артикул / цвет (через `resolveOrderNomenclature`);
- даты: дата заказа, срок сдачи (deadline-бейдж, «осталось N дн.»),
  ввод в производство (`order.inProductionAt`);
- комментарий менеджера (если задан);
...
Production facts: KPI стадий, маршрут операций (snapshot), размерный
breakdown, текущие stage buckets из `/api/shopfloor/state`, журнал
отгрузок. В самом конце вкладки — настроечные блоки, переехавшие из
удалённой вкладки «План»:

- «Цвета по строкам техкарты» (`view/order-material-colors-card.tsx`,
  `OrderMaterialColorsCard`) — primary-edit `selectedColorText`;
- «Очередь выдачи кроя по размерам» (`OrderCutIssueRulesCard`).
...
Вкладка «План» убрана. Куда переехало содержимое:

- продукт / лекало / артикул / цвет / даты — в шапку
  (`order-management-header.tsx`);
- маршрут / привязка к техкарте — во вкладку «Производство»;
- «Цвета по строкам техкарты» и «Очередь выдачи кроя» — в самый конец
  вкладки «Производство»;
- отдельная read-only таблица «План по размерам» убрана (размерный
  срез остаётся в производственной матрице).
