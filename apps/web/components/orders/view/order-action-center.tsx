...
function buildAlerts(
  ...
  } else if (status === 'SAMPLE_PRODUCTION') {
    // Этап «Производство сигнального образца»: образец в работе, тираж
    // ещё не запущен. Подсказываем менеджеру дальнейший шаг — полный
    // запуск тиража кнопкой «Запустить в производство» в шапке.
    alerts.push({
      id: 'sample-production',
      tone: 'info',
      icon: <Rocket size={16} strokeWidth={1.7} aria-hidden />,
      title: 'Идёт производство сигнального образца',
      hint: 'Тираж ещё не запущен. После проверки образца запустите весь тираж кнопкой «Запустить в производство» выше.',
      cta: { label: 'Открыть образец', href: orderTabHref('signalSample') },
    });
  }
