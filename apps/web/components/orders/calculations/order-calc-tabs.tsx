 ...
 * неактивных) + чип стадии — варианты сравниваются, не переключаясь.
 * Расчёт завершают ПО КАЖДОМУ варианту, поэтому чипов два: «черновик»
 * (не отправлен на расчёт) и «рассчитан» (по варианту есть завершённая
 * смета).
 *
 * `canSwitch=false` (заказ ЗАПУЩЕН В ПРОИЗВОДСТВО): переключение/
 * создание/удаление недоступны — вариант выбран, остальные вкладки
 * остаются как история просчёта.
 *
...
export function OrderCalcTabs({
  ...
  const onActivate = (calcId: string, title: string) => {
    ...
    const warning = warnUnsavedForm
      ? `Переключить на «${title}»? Несохранённые правки формы будут потеряны. Окно заказа покажет данные и себестоимость этого варианта; если расчёт по нему не завершён, заказ вернётся в статус «Расчёт».`
      : `Переключить на «${title}»? Окно заказа покажет данные и себестоимость этого варианта; если расчёт по нему не завершён, заказ вернётся в статус «Расчёт». Потребности и сметы других вариантов не затрагиваются.`;
  ...
  const lockHint = dto.canSwitch
    ? undefined
    : 'Вариант выбран: заказ запущен в производство. Остальные вкладки — история просчёта.';
          ...
          return (
            ...
            >
              ...
              >
                ...
                {/* Стадия ВАРИАНТА: черновик (не отправлен на расчёт —
                    считается кнопкой «Перевести в расчёт»/«Рассчитать
                    вариант») → отправлен → рассчитан (завершена смета
                ...
                ) : item.costEstimateCompletedAt != null ? (
                  <span
                    className="order-calc-tabs__stage order-calc-tabs__stage--done"
                    title="По варианту завершён расчёт себестоимости"
                  >
                    рассчитан
                  </span>
                ) : null}
