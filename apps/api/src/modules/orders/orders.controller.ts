 ...
 * участник производственного флоу, кому необходимо увидеть заказ, чтобы
 * выпустить по нему паспорт. Поэтому на GET-маршрутах список разрешённых
 * ролей расширен до `SHOP_MANAGER + CUTTER_ASSISTANT`.
 * Создание/редактирование/смена статуса по-прежнему доступны только
 * менеджерам.
 *
 * С той же мотивацией на `GET /orders/:id` добавлен `CUTTER`: раскройщик
 * теперь сам выпускает паспорта (вкладка «Выпуск» кабинета `/cutter`) и
 * сам правит только что выпущенный паспорт
 * (`/cutter/passports/[id]/edit`). Форме правки нужна размерная матрица
 * заказа (`items[].qtyPlan`) — без неё раскройщик не сможет перевести
 * паспорт на размер, по которому ещё нет ни одного выпуска. Тот же
 * `CUTTER` уже стоит на подресурсе `GET /orders/:id/passports`
 * (`OrderPassportsController`). Доступ read-only, запись раскройщику
 * по-прежнему закрыта.
 */
@Controller('orders')
