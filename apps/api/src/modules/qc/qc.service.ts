  ...
  private async loadDetail(passportId: string): Promise<QcPassportDetailDto> {
    ...
    if (lastQcPassed) {
      if (
        ...
        // Сканы по самой ОТК-категории из признака исключены намеренно
        // (инцидент 19.08.2026, заказ 02-00013): повторный скан
        // проверенного паспорта самим контролёром — это его же событие,
        // а карточка от него схлопывалась с текстом «паспорт ушёл на
        // следующую операцию». Паспорт при этом никуда не уходил, а
        // наоборот вставал контролёру на руки — и снять его она уже не
        // могла, потому что карточка не открывалась ни сканом, ни из
        // «В работе у вас». Сам повторный скан теперь no-op
        // (`PassportsService.scanOnOperation`), а этот фильтр
        // дополнительно расшивает уже накопленные паспорта: карточка
        // открывается, «Проверка выполнена» снимает владельца (см.
        // ветку освобождения в `completeQc` до идемпотентного выхода).
        const moved = await this.prisma.passportEvent.findFirst({
          where: {
            ...
            operation: { category: { not: OperationCategory.QC } },
          },
          select: { id: true },
        });
