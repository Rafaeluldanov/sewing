...
export class ShiftsService {
  ...
  async stop(dto: { employeeId: string }): Promise<ShiftSessionDto> {
    ...
    // Активный подкрой (`RecutSession`) здесь НЕ завершаем: «жёсткая
    // граница сменой или предупреждение мастеру» — решение №12 Аудита
    // движка расчёта 13.09.2026, владельцем не принято (ревью G4-3).
    // Забытый таймер ограничен предохранителем в `RecutService.complete`
    // и подсвечен флагами в DTO подкроя/мастера.
