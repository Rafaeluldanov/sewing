      ...
      if (otherTemplateRows === 0) {
        throw new ConflictException({
          ...
          message:
            ...
            'Чтобы начать с чистого листа, смените номенклатуру расцветки.',
        });
      }
    }
      ...
      throw new ConflictException({
        ...
        message:
          order.status === 'CANCELLED'
            ? 'Заказ отменён — состав материалов менять нельзя.'
            : 'Заказ завершён — состав материалов менять нельзя.',
      });
    }
  }
