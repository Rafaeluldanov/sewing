    ...
    const events = await this.prisma.passportEvent.findMany({
      where: {
        ...
        passport: {
          ...
          // ⛔ ЛЕГАСИ-ОЧЕРЕДЬ, выключена по существу (решение владельца 04.09.2026): единица
          // учёта — ДОКУМЕНТ ПРОИЗВОДСТВА, а не паспорт. Заказы из ERP сдаются очередью
          // `integrations/erp-production` целиком, а собственным заказам цеха ERP ответить
          // нечего — у неё нет ни их номенклатуры, ни места учёта. Фильтр оставлен взаимно
          // исключающим с новой очередью: паспорт не может уехать двумя путями сразу.
          order: {
            status: { not: 'CANCELLED' },
            erpCustomerOrderId: null,
          },
        },
      },
