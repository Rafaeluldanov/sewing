...
export class ErpProductionController {
   ...
   *
   * Курсор — `?ready_from=` по `queue_at` документа (max готовности и пересборки; аудит движка
   * расчёта 13.09.2026, G7-1, ревью: ключ курсора обязан быть ключом сортировки — см.
   * `ErpProductionService.listPending`). `closed_from` принимается как синоним: так ручка
   * называлась, пока очередь держалась на ответе ERP, и ломать её опрос ради переименования
   * параметра — плохой обмен. Без параметра — отсечка из настроек (`erpFinishedGoodsSince`).
   */
  @Get()
  async pending(
