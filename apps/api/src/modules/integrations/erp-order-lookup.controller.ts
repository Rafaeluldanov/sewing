...
export class ErpOrderLookupController {
   ...
   *
   * ⛔ Приезжает ПОЛНАЯ картина, а не дельта: план расцветок приводится к присланному тем же
   * путём, что у формы правки (`OrdersService.update` → `resyncColorwayDerived`), поэтому повтор
   * запроса безопасен — ERP шлёт его после молчания сети, не рискуя удвоить тираж. Аудит движка
   * расчёта 13.09.2026, G9-1/G6-1: расцветки сопоставляются по цвету и НЕ пересоздаются — нормы,
   * параметры, ручные строки и строки потребности цеха переживают досылку (меняется только
   * поразмерный план), а добор потребности не дублирует строки по расцветкам.
   *
