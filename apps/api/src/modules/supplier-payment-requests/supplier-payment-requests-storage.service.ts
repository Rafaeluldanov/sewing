...
import { SUPPLIER_PAYMENT_REQUEST_FILE_MAX_SIZE_BYTES } from '@sewing/shared/supplier-payment-requests';
...
/**
 * Локальный файловый storage для вложений заявки на оплату
 * (`SupplierPaymentRequestFile`).
 *
 * Зеркалит `ConstructorTasksStorageService`:
 *   - файлы лежат в
 *     `<uploadsRoot>/supplier-payment-requests/<requestId>/<safeName>`;
 *   - `safeName` = `${timestamp}-${randomHex}.${ext}` (не зависит от
 *     пользовательского `originalname`, защита от path-traversal);
 *   - формат файла НЕ ограничен (PDF / JPG / PNG / XLSX / ...),
 *     валидация только по размеру;
 *   - раздача — единым префиксом `/uploads` (см. `main.ts`).
 */
@Injectable()
export class SupplierPaymentRequestsStorageService {
  private readonly logger = new Logger(
    SupplierPaymentRequestsStorageService.name,
  );
  ...
  /**
   * Сохранить один файл заявки. Возвращает данные для записи в
   * `SupplierPaymentRequestFile`.
   */
  ...
  // -------------------------------------------------------------------------
  // INTERNAL helpers (parallel to ConstructorTasksStorageService).
  // -------------------------------------------------------------------------
  ...
  private assertSize(file: UploadedFileLike): void {
    if (file.size > SUPPLIER_PAYMENT_REQUEST_FILE_MAX_SIZE_BYTES) {
      const mb = Math.round(
  ...
  /** Удалить физический файл по публичному URL (молча игнорирует ENOENT). */
  async deleteByPublicUrl(publicUrl: string): Promise<void> {
    ...
    if (!absolute.startsWith(this.uploadsRoot + '/')) return;
    try {
