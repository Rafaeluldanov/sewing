...
export type AuditEntityType =
   ...
   * после фиксации и изменил сумму), `PRODUCTION_DOCUMENT_BACKFILLED`
   * (собран кнопкой по уже закрытому заказу — обычно закрытому до
   * появления раздела), `PRODUCTION_DOCUMENT_SYNCED` (человек нажал
   * «обновить по фактам»). `entityId` — `ProductionDocument.id`.
   * Документ не проводят и не подтверждают, поэтому «почему сумма
   * поменялась» восстанавливается только по этим событиям — включая
   * ответ «потому что нажали обновление».
   */
  | 'PRODUCTION_DOCUMENT'
  | 'QC'
  | 'WTO'
