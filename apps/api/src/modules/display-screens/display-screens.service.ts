...
export class DisplayScreensService {
  ...
  async list(): Promise<DisplayScreenListItemDto[]> {
    const rows = await this.prisma.displayScreenConfig.findMany({
      ...
      include: { employee: { select: { id: true, login: true } } },
    });
    return rows.map((r) =>
      toListDto({
        id: r.id,
   ...
   *      открытую транзакцию.
   *   2. `$transaction([Employee.create, DisplayScreenConfig.create])`
   ...
   *   3. `P2002` на `Employee.login` транслируется в стабильный
    ...
    try {
      const result = await this.prisma.$transaction(async (tx) => {
        ...
        const config = await tx.displayScreenConfig.create({
          data: {
            name: dto.name,
            division: dto.division as OrderDivision,
