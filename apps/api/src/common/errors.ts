...
// ═══════════════════════════════════════════════════════════════════════════
// ПРАВИЛО (зафиксировано Рафаэлем 04.09.2026, МЕНЯТЬ ТОЛЬКО ПО ЯВНОЙ ПРОСЬБЕ):
// ЗАКАЗ, РОЖДЁННЫЙ ЗАКАЗОМ ПОКУПАТЕЛЯ ERP, В ЦЕХЕ НЕ ПРАВИТСЯ И НЕ ЗАКРЫВАЕТСЯ
// НЕДОВЫПУЩЕННЫМ. Такой заказ (`Order.erpCustomerOrderId` заполнен) — зеркало
// строк заказа покупателя: тираж и состав приехали снаружи, и цех о них не
// договаривается. Правило ERP — `docs/kb/sewing.md` §0.10 (там же перечислены
// закрытые пути со стороны ERP), полнота — оттуда же: «заказ на 30 — значит 30,
// цех дошивает».
//
// ПОЧЕМУ. У ERP заблокированы строки отправленного заказа: их нельзя ни
// изменить, ни удалить, а реализация ждёт ПОЛНОГО прихода. Если цех уменьшит
// тираж или закроет заказ недовыпущенным, приход придёт меньше заказанного —
// и заказ покупателя не отгрузится уже никогда: сдача необратима (заказ
// исчезает из очереди `erp-production` вместе с ответом ERP), а недостающее
// приехать больше не может.
//
// НЕ ОТМЕНЯЕТ: правку маршрута и операций (технология — дело цеха), брак и
// перекрой (цех дошивает), правку собственных заказов цеха — на них ничего из
// этого не распространяется.
//
// НЕ УБИРАТЬ.
// ═══════════════════════════════════════════════════════════════════════════

/** Попытка изменить тираж/состав заказа, рождённого заказом покупателя ERP. */
export class ErpOrderPlanLockedException extends ConflictException {
  constructor(
    what = 'План',
    ...
    super({
      statusCode: 409,
      code: 'ERP_ORDER_PLAN_LOCKED',
      message:
        `${what} этого заказа задан заказом покупателя в ERP` +
        (orderNumber ? ` (${orderNumber})` : '') +
        ' и в цехе не меняется. Правьте в ERP — оттуда заказ приедет заново.',
    });
  }
}

/** Попытка закрыть ERP-заказ, у которого упаковано меньше, чем заказано. */
export class ErpOrderUnderproducedException extends ConflictException {
  constructor(packed: number, plan: number) {
    super({
      statusCode: 409,
      code: 'ERP_ORDER_UNDERPRODUCED',
      message:
        `Упаковано ${packed} из ${plan}. Заказ из ERP закрывается только целиком: ` +
        'недостающее надо дошить — после сдачи допоставить уже нечем.',
    });
  }
}

/** Повторное создание заказа по тому же заказу покупателя ERP. */
export class ErpOrderAlreadyLinkedException extends ConflictException {
  constructor(orderNumber?: string | null) {
    super({
      statusCode: 409,
      code: 'ERP_ORDER_ALREADY_LINKED',
      message:
        'По этому заказу покупателя ERP заказ цеха уже создан' +
        (orderNumber ? `: ${orderNumber}` : '') + '.',
    });
  }
}
...
/**
 * Сотрудник пытается работать с коробками, но активная смена не на
 * операции с признаком `Operation.boxPacking` (см.
 * `PackingService.assertPackingActor`). На MVP это soft-validation:
 * блокируем только мутации (создание коробки, добавление, закрытие).
 * Просмотр списка коробок открыт всем.
 *
 * Формулировка намеренно говорит про упаковку В КОРОБКИ, а не «операцию
 * категории Упаковка»: в категории `PACKING` у клиента живёт ещё и
 * «Распаковка» (приёмка приходящего сырья, первый шаг маршрута). Пока
 * текст был про категорию, упаковщик со сменой на «Распаковке» читал
 * его как «смена же на упаковке, всё правильно» и не понимал, что
 * взял не ту операцию.
 */
export class PackingShiftRequiredException extends BusinessException {
  constructor() {
    super(
      ...
      'Нужна активная смена на операции упаковки в коробки. Проверьте, какую операцию вы взяли: приёмка/распаковка сырья коробки не собирает.',
      HttpStatus.CONFLICT,
    );
  }
}
...
/**
 * Отсканирован «Мой QR-код» сотрудника (`SEWING_EMPLOYEE:<token>`), но
 * подпись не сошлась или срок жизни истёк.
 *
 * Отдельный код от `INVALID_EMPLOYEE_QR` намеренно: там «вы поднесли не
 * тот QR» (паспорт, ячейка), а здесь QR правильный — просто протух.
 * Токен живёт 12 часов, лечится открытием «Мой QR-код» заново, и текст
 * ошибки должен вести именно туда, а не заставлять искать другой бейдж.
 */
export class EmployeeQrTokenInvalidException extends BusinessException {
  constructor() {
    super(
      'EMPLOYEE_QR_TOKEN_INVALID',
      'QR сотрудника просрочен или повреждён — попросите открыть «Мой QR-код» заново.',
      HttpStatus.BAD_REQUEST,
    );
  }
}
...
/**
 * Попытка увести паспорт ВПЕРЁД с швейного шага, на котором он стоит
 * прямо сейчас и который НЕ закрыт (`OPERATION_FINISHED` по нему нет).
 * Типичный случай — ОТК сканирует паспорт, взятый швеёй пару минут
 * назад: инцидент 31.08.2026, заказ 02-00020, где контролёр забрала у
 * распошивщицы пять паспортов подряд.
 *
 * Чем отличается от соседей:
 *   - `PASSPORT_PRECEDING_STEP_INCOMPLETE` — перепрыгнули шаг, лежащий
 *     СТРОГО МЕЖДУ текущим и целевым;
 *   - `PASSPORT_PARALLEL_GROUP_INCOMPLETE` — AND-гейт по группе;
 *   - здесь — шаг, на котором паспорт СТОИТ. Ровно он и не проверялся
 *     до 01.09.2026, потому что не попадал ни в один из интервалов.
 *
 * Почему это 409, а не предупреждение: без `OPERATION_FINISHED` не
 * создаётся `OperationEntry`, то есть сделка за уже сделанную работу не
 * начисляется никому и никогда. Разбор — в
 * `PassportsService.evaluateRouteOrder::currentStepCandidate`.
 *
 * Гейт держит ТОЛЬКО перехват из рук (`currentEmployeeId != null`):
 * паспорт, лежащий в буфере с незакрытым шагом, идёт дальше как
 * раньше — иначе брошенный в конце смены крой залипал бы до мастера.
 *
 * Текст обязан называть выход, иначе цех встанет: операцию завершает
 * сама швея (в том числе из секции «Не закрыто вами» на `/work`, если
 * паспорт уже унесли), либо мастер снимает владельца («Снять с
 * сотрудника», `MasterActionsService.unassign`) и поток идёт дальше.
 */
/**
 * Сотрудник просит закрыть свою «незакрытую» операцию по паспорту
 * (`POST /passports/:id/close-unclosed-operation`), но открытого
 * назначения по этому паспорту у него нет: он его не брал, уже закрыл,
 * либо взятая операция не швейная (`OPERATION_FINISHED` пишут только
 * `SEWING` — см. `route-debt.ts`).
 *
 * Отдельный код, а не `PASSPORT_NOT_YOURS`: там речь про «паспорт не на
 * твоих руках», и это здесь как раз НОРМА — весь смысл ручки в том,
 * чтобы закрыть работу по паспорту, который уже увели.
 */
export class PassportNoUnclosedWorkException extends BusinessException {
  constructor() {
    super(
      'PASSPORT_NO_UNCLOSED_WORK',
      'По этому паспорту у вас нет незакрытой операции: вы её либо не брали, либо уже закрыли.',
      HttpStatus.CONFLICT,
    );
  }
}

export class PassportCurrentStepIncompleteException extends BusinessException {
  constructor(operationName: string | null, employeeName: string | null) {
    const what = operationName
      ? `операция «${operationName}»`
      : 'операция текущего шага';
    const who = employeeName ? `, паспорт в работе у ${employeeName}` : '';
    super(
      'PASSPORT_CURRENT_STEP_INCOMPLETE',
      `Нельзя забрать паспорт: на нём не завершена ${what}${who}. Сначала операцию нужно завершить — или мастер снимет паспорт с сотрудника.`,
      HttpStatus.CONFLICT,
    );
  }
}
...
/**
 * Документ выпуска по ОТМЕНЁННОМУ заказу не заводится.
 *
 * Сформировать документ можно на любой стадии заказа (решение владельца 14.09.2026): по
 * открытому заказу он накапливает выпуск и остаётся «формируется», пока заказ не закроют.
 * Отмена — единственная стадия, где выпуска не будет уже никогда: приходовать его нечем.
 */
export class ProductionDocumentOrderCancelledException extends BusinessException {
  constructor() {
    super(
      'PRODUCTION_DOCUMENT_ORDER_CANCELLED',
      'Заказ отменён — выпуска по нему не будет, документ не заводим.',
      HttpStatus.CONFLICT,
    );
  }
}
...
/**
 * Заказ ЗАКРЫТ, а упакованных паспортов с годным выпуском по нему нет.
 *
 * Пустой документ выпуска по закрытому заказу — это не документ: он сообщал бы, что цех что-то
 * сдал, хотя сдавать было нечего. Такие заказы (закрытые без упаковки) остаются без документа
 * осознанно. К открытому заказу не относится: там пустой документ — нормальное начало,
 * упаковка ещё впереди.
 */
export class ProductionDocumentNothingReleasedException extends BusinessException {
  constructor() {
    super(
      'PRODUCTION_DOCUMENT_NOTHING_RELEASED',
      'Заказ закрыт без упакованных паспортов — выпускать нечего, документ пустым не заводим.',
      HttpStatus.CONFLICT,
    );
  }
}
 ...
 * `amountToPayRub < 0` (удержание больше начислений). Раньше `pay`
 ...
 * менеджер уменьшает удержание до суммы не больше начислений, остаток переносит
 * в следующую ведомость. Ревью K1: полный зачёт «в ноль» (нетто ровно 0) сюда
 * не попадает — `pay` проводит его выплатой на 0 ₽. Строка без начислений с
 * одним удержанием тоже не попадает (повторно брать нечего) и ведёт себя как
 * раньше.
 */
