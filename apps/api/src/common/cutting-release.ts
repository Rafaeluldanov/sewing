 ...
 * (`releasedPairs >= totalPairs`).
 *
 * ВАЖНО (частичное завершение раскроя): требование `status = 'DONE'`
 * оставлено СОЗНАТЕЛЬНО и больше не совпадает со статусом строки в
 * `listReadyForRelease` — там готовность считается по закрытым раскладам,
 * и заказ может быть «выпущен весь» по ним, пока раскройщик настилает
 * следующий расклад. Авто-завершение заказа при упаковке
 * (`PackingService.maybeCompleteOrderOnPack`) обязано быть строже: пока
 * раскройщик не нажал «Раскрой завершён», он ещё может добавить расклад,
 * и закрывать заказ рано. Цена решения: если тираж прошит и упакован до
 * «Раскрой завершён», заказ дождётся этой кнопки (или ручного
 * завершения) — это лучше, чем закрыть заказ, по которому ещё режут.
 *
