...
безусловный. Аудит движка расчёта 13.09.2026, L1-10: он же стоит и на
«сделке по размерам» (`pricingModeOverride = BY_SIZE`) — ставка нужна по
КАЖДОМУ размеру плана заказа: из присланного поразмерного набора (он
replace-all), иначе из снимка, иначе из справочника операции
(`OperationRateBySize`); в сообщении перечислены размеры без ставки.
Ревью L1-10: этот гард бьёт только по ИЗМЕНЕНИЮ — режим шага стал
`BY_SIZE` или правили его набор поразмерных ставок; шаг, уже стоявший
`BY_SIZE` без ставок (принят до гарда), при replace-all формы проходит
как раньше — с предупреждением плана операций, — и не запирает правку
соседних операций. Тот же гард зеркалится в фазе 0 активации варианта
просчёта (V1-5), до мутаций.
...
При повторе операции в маршруте (ОТК/ВТО до и после) carry снимка шагов
при смене шаблона отдаёт подряд только ПЕРВОМУ вхождению (ключ —
`operationId`, объём иначе задвоился бы), а восстановление варианта
просчёта ключует записи снимка по `(operationId, № вхождения)` — каждая
достаётся ровно одному шагу (Аудит движка расчёта 13.09.2026, V1-4,
ревью; см. ADR-0023 §2.6). Снятие метки гасит и цену, и все поразмерные
объёмы шага.
