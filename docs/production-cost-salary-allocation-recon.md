...
> **Поправка (аудит движка расчёта 13.09.2026, F1-5):** премисса «на ОТК
> нет `OPERATION_SCAN`» верна для швей, но не для терминалов ОТК/ВТО —
> они берут паспорт сканом (`PassportsService.scanOnOperation` →
> `OPERATION_SCAN` с `operationId` операции QC/IRONING), и
> `PassportRealCostService` с 13.09.2026 читает этот скан как accept:
> интервал `[скан..QC_PASSED/WTO_PASSED]` точный, а не «по разрыву».
> Простой при этом считается как `оплачено − разнесено`
> (`SalaryEntry.workedSeconds` / закрытые смены месячника, F1-1 / F1-2), а
> не `SHIFT_MINUTES − tracked`.
