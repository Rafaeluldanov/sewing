...
- Статус: Принято (ревизия 2026-06: почасовая оплата)
- Дата: 2026-04
- Контекст: пост-ADR-0020, расширение зарплатного контура на окладные роли

---

## Ревизия 2026-06: переход на ПОЧАСОВУЮ оплату

Исходное решение (ниже) платило **плоскую ставку за смену**
(`Employee.salaryPerShift`) по факту, что в день была хотя бы одна
`ShiftSession` — длительность и закрытие смены не учитывались. На
практике сотрудники работают не по фиксированным сменам, а «на время»,
поэтому плоская ставка давала неверные суммы.

Изменение (см. `SalaryService.syncDailySalary`):

- ставка теперь почасовая — `Employee.salaryPerHour` (₽/час); legacy
  `salaryPerShift` сохранён в БД, но в расчёте не участвует;
- дневная сумма = `Σ(длительности ЗАКРЫТЫХ ShiftSession за день в сек) /
  3600 × salaryPerHour`, округление до копеек; отработанные секунды
  пишутся в `SalaryEntry.workedSeconds`;
- открытая смена (`endedAt = null`) в расчёт не идёт. Если за день нет
  ни одной закрытой смены — запись не создаётся (дисциплина закрытия;
  иначе менеджер правит сумму руками). Несколько закрытых смен за день
  суммируются — активная смена у человека одна (partial-unique индекс),
  перекрытий нет;
- `reset = true` пересчитывает сумму по той же почасовой формуле, а не
  по плоской ставке;
- контур разнесения оклада на себестоимость (`computeMinuteRate` в
  `costs`/`dashboard`) переведён с `salaryPerShift / SHIFT_MINUTES` на
  `salaryPerHour / 60` (численно совпадает при бэкфилле
  `salaryPerHour = salaryPerShift / 8`, т.к. `SHIFT_MINUTES = 480`);
- бэкфилл существующих окладников: `salaryPerHour = salaryPerShift / 8`
  (миграция `20260829100000_add_salary_per_hour`).

Всё остальное (отдельная таблица `SalaryEntry`, `editedManually`,
lock-by-payout, source `SHIFT_DAY`/`MANUAL`) — без изменений.
