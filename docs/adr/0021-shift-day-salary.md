...
### Дополнение 2026-09-13: предохранитель на длительность смены (аудит K7, G4-3)
...
строка `SHIFT_DAY` платила их целиком (73 ч × 300 ₽ = 21 900 ₽ за
«день»). Автозакрытие смен опционально, по умолчанию выключено, ручной
`stop` его порог не применяет, а уже закрытые смены оно не правит.

- В `computeWorkedSeconds` (`salary.service.ts`) длительность КАЖДОЙ
  закрытой смены ограничена сверху: предел =
  `CompanySettings.shiftMaxDurationHours`, если задан (> 0), иначе
  **16 ч** (`apps/api/src/modules/salary/shift-worked-cap.ts`). Несколько
  смен за день суммируются по-прежнему, каждая — не больше предела.
  Та же функция считает справочные часы `MONTH_SALARY` и пересчёт
  `reset = true`.
- Сама `ShiftSession` не меняется — табель мастера и аналитика видят
  настоящую длительность; режется только число в деньгах ведомости.
- Обрезка не молчит (ревью K7): строка `SHIFT_DAY` получает
  `managerComment = «Обрезано предохранителем N ч (фактически M ч)»`,
  срабатывание пишется в лог (`event=salary.capped`). Пометка — своя
  (узнаётся по префиксу, `shift-worked-cap.ts::CAP_NOTE_PREFIX`): sync
  ставит и снимает только её, текст менеджера не трогает; `reset`
  стирает комментарий и ставит пометку заново, если предохранитель
  сработал. Через снимок строки выплаты пометка доезжает до ведомости
  выплаты. Справочные часы `MONTH_SALARY` режутся тем же пределом, но
  без пометки — на сумму оклада они не влияют.
- Тот же предел применяется к подкрою (`RecutSession`): `workedSeconds`
  подкроя не превышает предел на смену. Забытый таймер подкроя иначе
  платил все календарные часы до «Завершить» (пт 16:00 → пн 09:00: 65 ч
  × 300 ₽ = 19 500 ₽ за 2 ч работы) — и в ведомость (`RECUT`), и в
  `recut_rub` документа выпуска для ERP. **Концом смены подкрой НЕ
  режется и закрытием смены НЕ завершается**: «жёсткая граница сменой или
  предупреждение мастеру» — решение №12 из списка решений владельца по
  аудиту 13.09.2026, оно не принято (ревью G4-3). До решения — только
  предохранитель и предупреждение: флаги `longerThanShift` /
  `cappedByGuard` в `RecutSessionDto` (доска раскройщика),
  `activeRecutOverLimit` в `MasterActiveShiftDto` (режим «Активные» у
  мастера) и пометка в `managerComment` строки `RECUT` («Подкрой длиннее
  смены; обрезано предохранителем N ч (фактически M ч)»).
- Отчёты себестоимости (`costs/shift-presence.ts`, F1-2): оплаченные
  минуты месячника по закрытым сменам режутся тем же пределом, что и
  часы почасовика в `SalaryEntry.workedSeconds` — простой считается по
  одному правилу с ведомостью.
