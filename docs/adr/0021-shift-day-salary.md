...
- Тот же предел применяется к подкрою (`RecutSession`): конец подкроя —
  не позже конца своей смены, закрытие смены (`ShiftsService.stop`, в
  т. ч. мастером) завершает активный подкрой тем же моментом, а
  `workedSeconds` подкроя не превышает предел на смену. Забытый таймер
  подкроя иначе платил все календарные часы до «Завершить»
  (пт 16:00 → пн 09:00: 65 ч × 300 ₽ = 19 500 ₽ за 2 ч работы) — и в
