...
> **Актуально с 7adac18 (17.06.2026), уточнено аудитом движка расчёта
> 13.09.2026 (F1-3 / F1-11):** алгоритм подключён —
> `salaryAllocatedCostRub` = разнесённое реальное время окладников
> (`PassportRealCostService`) по паспортам, выпущенным в периоде, на окне
> самого паспорта (минуты ОТК/ВТО до `dateFrom` не теряются); входит в
> `totalCostRub`. Ревью F1-3: окно двигают только завершения окладных
> операций (QC/IRONING/PACKING/CUTTING либо сотрудник с окладом), назад —
> не дальше 60 дней от `dateFrom` (иначе `warnings`: «окно разноса
> ограничено…»), норма часов месячника берётся по месяцу дня события. Предупреждение теперь ключуется на оклад по паспортам,
> НЕ выпущенным в периоде (с суммой), а не на наличие сдельной.
> `salaryWorkingCostRub` шапки считается по всем паспортам с событиями в
> окне и пересекается с allocated — складывать их нельзя.
...
| `salaryAllocatedCostRub` | разнесённый оклад выпущенных паспортов (окно паспорта; с 7adac18, F1-3) | Факт времени окладников | «Оклад (разнесён на выпуск)»; warning только про оклад по не выпущенным паспортам (F1-11) |
...
- **Не реюзаем `salaryCost` из старого CostsService** — там логика
  ...
  *(Устарело: с 7adac18 оба отчёта используют один движок
  `PassportRealCostService`, см. врезку в §2.)*
