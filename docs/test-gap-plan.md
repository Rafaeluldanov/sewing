...
| P0-2 | `PackingService.close` — нет проверки идемпотентности и эффекта на `OperationEntry`. | **NEW** `tests/integration/packing-close-idempotent.test.ts` | Двойной клик «закрыть короб» в UI не должен дублировать перевод `OperationEntry` в `APPROVED` и не должен писать второй `BOX_CLOSED` в AuditLog. Без теста — потенциальное двойное начисление. | (1) `close` на закрытом коробе возвращает идемпотентный 200 либо 409 (зафиксировать какое — это finding); (2) количество `OperationEntry` со статусом `APPROVED` после второго `close` равно количеству после первого; (3) `AuditLog` имеет ровно одну запись `BOX_CLOSED`/`PASSPORT_PACKED`-серию по этому коробу; (4) `Box.closedAt` не сдвигается. |
| P0-3 ✅ | `ShiftsService` — нет dedicated integration теста; покрытие косвенное через 18 других тестов. | **DONE** `tests/integration/shifts.test.ts` (12 тестов). FINDING зафиксирован в `docs/operations-test-findings.md`: `start` не проверяет `Equipment.allowedOperations` — backend-enforce отсутствует, UI/`shifts/meta` allow-list работает. | `start/stop`, partial-unique constraint (одна активная смена на сотрудника), вызов `SalaryService.syncDailySalary`, `/shifts/meta` allow-list — все проверяются мимоходом, без негативных кейсов. | covered: (1) повторный `start` → 409 `SHIFT_ALREADY_ACTIVE`; (2) FINDING-test пинит текущее поведение по equipment-operation binding (см. `docs/operations-test-findings.md`); (3) `stop` без активной → 409 `SHIFT_NOT_ACTIVE`; (4) salary trigger — оставлен в `salary.test.ts §1..§4`, не дублируется; (5) `getCurrent` без/со сменой; (6) `getCurrentWork` фильтр по `PassportStatus != IN_PROGRESS` (PACKED-кейс); (7) bad equipmentId → 404, bad operationId → 404, inactive equipment/operation → 409; (8) 401 без cookie на все 5 эндпоинтов. |
...
| P0-5 | QC — нет идемпотентности `completeQc × 2` и негативного `recordDefect` (`qty > qtyGood`). | **EXTEND** `tests/integration/qc-shift-flow.test.ts` | Существующий файл уже устаканился на shift-gate; добавить туда же два кейса логически близко и не плодит новый файл. | (1) повторный `POST /api/qc/passports/:id/complete` → 200 идемпотентно, в `PassportEvent` ровно одна запись `QC_PASSED`, в `AuditLog` ровно одна `QC_COMPLETED`; (2) `POST /api/qc/passports/:id/defects` с `qty > Passport.qtyGood` → 400, `Passport.qtyGood/qtyDefect` не меняется, `PassportDefect` не создаётся. |
| P0-6 | WTO — нет негативного «WTO до QC → 409». | **EXTEND** `tests/integration/wto-shift-flow.test.ts` | Сейчас файл проверяет только shift-gate и happy-path. Гейт `QC_PASSED` — основной инвариант стадии WTO. | (1) `POST /api/wto/passports/:id/complete` без события `QC_PASSED` → 409 `PASSPORT_NOT_QC_PASSED`, в `PassportEvent` нет `WTO_PASSED`; (2) повторный `complete` после первого успешного — 200 идемпотентно, ровно одна запись `WTO_PASSED`. |
| P0-7 | UI `/orders/[id]/passports/new` для CUTTER_ASSISTANT — регрессия на admin-only `GET /api/employees`. | **NEW** `tests/smoke/orders-passports-new-cutter-assistant.smoke.test.ts` | RECON §6 инвариант 8: CUTTER_ASSISTANT не должен дёргать admin-only endpoint, иначе форма ломается с 403. Это финальный «production hot-spot» из coverage. | (1) Загрузка страницы под CUTTER_ASSISTANT не делает fetch на `/api/employees`; (2) форма создания паспорта рендерит cutter-секцию без выбора (auto-attrib creator=CUTTER не применим, должен быть либо selfId, либо отдельный «cutters»-эндпоинт); (3) submit формы → `POST /api/passports` 200. |
