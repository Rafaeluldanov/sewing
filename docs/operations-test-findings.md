...
| `ShiftsService.start` (allow-list enforcement) | `apps/api/src/modules/shifts/shifts.service.ts` — после `equipment.active`/`operation.active` добавлен `prisma.equipmentOperation.findFirst({equipmentId, operationId, isActive: true})`. Если связи нет — `ShiftOperationNotAllowedForEquipmentException` (`SHIFT_OPERATION_NOT_ALLOWED_FOR_EQUIPMENT`, 409). Источник правды совпадает с фильтром в `getMeta` (`isActive=true && operation.active`). Новая ошибка добавлена в `apps/api/src/common/errors.ts`. | `tests/integration/shifts.test.ts` → `start с operationId вне Equipment.allowedOperations → 409 SHIFT_OPERATION_NOT_ALLOWED_FOR_EQUIPMENT` (mismatch `qc-station-01` × `CUT_DIVISION` теперь 409, ShiftSession не создаётся) + `start с EquipmentOperation isActive=false → 409` (soft-delete биндинга через `updateMany`). | UI-фильтр `/shifts/meta` всегда работал; backend теперь enforce-ит ту же модель и блокирует прямой POST в обход UI. Существующие коды `EQUIPMENT_NOT_FOUND` / `OPERATION_NOT_FOUND` / `EQUIPMENT_INACTIVE` / `OPERATION_INACTIVE` / `SHIFT_ALREADY_ACTIVE` не затронуты — новая проверка стоит после exists/active checks и перед `findActiveByEmployee`. |
