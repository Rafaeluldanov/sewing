...
### F4b. Пакетное завершение операций швеёй («Завершить выбранные»)

Швея часто сначала набирает крой (сканирует много паспортов на
«Получить крой»), а к завершению переходит позже, разом по всей пачке.
Чтобы не сканировать каждый паспорт повторно для завершения, в блоке
«Текущий крой» (`current-work-card.tsx`) есть мультивыбор: чекбоксы на
карточках + «Выбрать все» + кнопка «Завершить выбранные (N)». Паспорта
в этом списке уже закреплены за швеёй (`currentEmployeeId = me`), поэтому
повторный скан-сверка не нужны — вместо него модалка-сводка «закроется N
паспортов · M шт».

`POST /api/passports/batch/complete-operations` (тело `{ passportIds:
string[] }`, владелец из сессии) → `PassportsService.completeOperationsBatch`.
Здесь **нет новой бизнес-логики**: каждый паспорт проходит через тот же
`completeOperationByEmployee` (своя транзакция, `OPERATION_FINISHED`,
PENDING-начисление, аудит, выпуск ГП, проверки маршрута). Завершение
идёт на `session.operationId`, как и одиночный flow.

**Партиальный успех** — сознательное решение: если один паспорт упал
(`PASSPORT_COMPLETE_BACKWARD`, `PASSPORT_NOT_YOURS`,
`PASSPORT_NOT_IN_PROGRESS`, незакрытая параллельная группа), остальные
всё равно закрываются. Ответ — `{ completed: [{passportId, number}],
failed: [{passportId, number, error, code}] }`. UI показывает сводку
«закрыто N, не закрыто M» и подсвечивает упавшие карточки причиной;
закрытые уходят из «Текущий крой» после `revalidatePath('/work')` +
`router.refresh()`. Одиночный скан-flow «Завершить операцию» остаётся
без изменений — пакет добавлен рядом. Тесты:
`tests/integration/passports-complete-operation-batch.test.ts`.
