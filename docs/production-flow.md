...
- Если паспорт УЖЕ засчитан в очередь (по `passportId` есть
  ...
  один раз. Ревью G3-1: проверка читает AuditLog по `entityType` +
  `entityId ∈ {все строки правила заказа}` (полный индекс
  `[entityType, entityId]`) и повторяется в `consumeInTx` внутри
  транзакции выдачи (за row-lock'ом паспорта) — двойной клик «Взять
  крой» второй раз не засчитывается. На `/work` модалка «Не тот размер»
  даёт «Всё равно взять»: паспорт, уже засчитанный в очередь (возврат
  от ОТК на первый шаг), клиентская сверка размера с баннером отличить
  не может — решает сервер (201 либо 409).
