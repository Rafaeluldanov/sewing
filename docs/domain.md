...
- `number` (uniq). Формат — по правилам ERP (решение владельца 07.09.2026):
  `ПРФ-NNNNNN`, где `ПРФ` — префикс организации ERP из настроек цеха
  (`CompanySettings.prefix`), счёт сквозной внутри префикса. Пока префикс не
  задан, работают прежние схемы: `КОД-NNNNN` по коду подразделения и суточная
  `O-YYYYMMDD-NNNN` без него. Существующие заказы не перенумеровываются
  (`OrderNumberService`),
...
Кроме структуры маршрута (`index`, `parallelGroup`) шаг несёт
**переопределения на этот заказ**: `rateOverride` (сдельная расценка),
`timeNormSecOverride` (норма времени), `pricingModeOverride` (оклад ⇄
сделка) и поразмерные строки `OrderRouteStepSizeOverride`
(`rate` / `seconds`). Пустое поле значит «по справочнику операции», а
не «нет»: действующее значение всегда `override ?? Operation.<поле>`.
Правятся одной ручкой `PUT /api/orders/:id/route-overrides`
(`docs/api.md §13`), справочник `Operation` при этом не трогается.
Сюда же добавлена метка сторонних услуг — §2.4a.
...
### 2.4a Сторонние услуги: `outsourced` на шаге заказа

Источник: ADR-0023, `prisma/schema.prisma::OrderRouteStep.outsourced`,
`OrderOperationPlanService.computeTotals`, `docs/order-flow.md §5.1`.

Решение владельца 10.09.2026: «если мы указали, что это будет делаться
на стороне, тогда мы не берём стоимость операций, а считаем стоимость
стороннего размещения. И на самих операциях нужно отметить, что делаем
на стороне. И в план-факте тоже достаточно метки».

Поля шага:

- `outsourced: Boolean @default(false)` — операцию этого заказа
  полностью или частично выполняет подрядчик;
- `outsourcePriceRub: Decimal(12,2)?` — цена размещения за ОДНО
  изделие (₽/шт), а не за единицу услуги подрядчика: на сторону
  отдают тираж;
- `OrderRouteStepSizeOverride.outsourcedQty: Int?` — сколько штук
  этого размера отдано.

**Объём.** Ни одного заданного `outsourcedQty` при `outsourced = true`
⇒ на стороне ВЕСЬ тираж операции. Задан хотя бы один ⇒ на стороне
ровно перечисленные штуки, остальное цех считает своей расценкой;
`outsourcedQty = 0` — «этот размер шьём сами» (набор всё равно
считается расписанным). Потолок — план по размеру (СУММА `qtyPlan`
всех строк заказа с этим `sizeId`), больше нельзя: 400
`ORDER_ROUTE_OUTSOURCED_QTY_OVER_PLAN`.

**Деньги.** По отданному объёму своя расценка (любой из трёх
`PricingMode`) в план НЕ идёт — вместо неё
`outsourcePriceRub × штуки`. Полное правило деления тиража и
поведение warnings (в том числе «Не задана цена стороннего
размещения…») — `docs/order-flow.md §5.1`. `Order.operationCostPlanRub`
остаётся ПОЛНЫМ планом операций, а `Order.operationOutsourceCostPlanRub`
— расшифровкой «в том числе»; складывать их нельзя.

**Почему метка на заказе, а не в справочнике `Operation`.** Подряд —
решение по конкретному тиражу, а не свойство технологии; справочник
общий на все заказы и его читает payroll
(`OperationsService.resolveRate`), так что флаг там означал бы «на
стороне везде и сразу». Снимок маршрута, наоборот, уже хранит решения
«про этот тираж» (§2.4) и после запуска иммутабелен.

⛔ **Метка — только про деньги и подпись в план-факте.** Шаг из
маршрута не исчезает, `operationTimePlanSec` считается по полному
тиражу, а паспорта, доска, гейты ОТК/ВТО/упаковки и сдельные
начисления её не читают вовсе: часть тиража по той же операции цех
делает сам, и «пропуск шага» остановил бы паспорта (упаковка считает
число шагов QC/IRONING в маршруте). Зарплата вычитания не требует —
начисление рождается из скана, а по отданному объёму сканов не будет.
Подробности и отвергнутые альтернативы — ADR-0023 §2.3, §3.

**Переживает пересборку.** `syncOrderRouteStepsSnapshot` пересоздаёт
шаги через `deleteMany + createMany`, поэтому метка, цена и
поразмерный объём переносятся carry-механикой: в шаблоне маршрута
подряда нет и взяться ему при пересоздании неоткуда. Снимок варианта
просчёта (`OrderCalculation`) тоже несёт эти поля, а шагам вне снимка
при активации варианта пишется явный `outsourced = false` — иначе
подряд соседнего варианта протёк бы через тот же carry.

Подрядчика, срок и документ закупки цех не хранит: это ERP
(заказ поставщику на номенклатуру-услугу, `docs/kb/sewing.md §6` в
репозитории `~/service`). Не путать с `OrderOutsourceRequirement`
(§3.4) — там своя ось.
...
Полного enforcement-а порядка нет (операция вне маршрута — вопрос
политики `CompanySettings.offRouteWorkPolicy`), но направление
движения гейты держат:
...
**Незакрытый шаг позади — доделка, а не откат** (каналы ВЗЯТИЯ:
`issue` / `scan`). Шаг, оставшийся позади без `OPERATION_FINISHED`
(мастер снял сотрудника, не закрыв операцию, — `unassign` намеренно
не трогает `currentRouteStepIndex`), раньше становился недостижимым:
вперёд его не проверял никто (гейт «пропуска» смотрит только интервал
между текущим и целевым шагом), а назад держал
`PASSPORT_ISSUE_BACKWARD` / `PASSPORT_SCAN_BACKWARD`. Теперь любой
сотрудник на этой операции берёт такой паспорт сам, без мастера:
паспорт переезжает на шаг долга, работа закрывается штатным
`complete` и попадает в историю и в начисления.

Ограничения правила (`evaluateRouteOrder`, `catchUpCandidate`):

- только `SEWING` вне параллельной группы — `OPERATION_FINISHED`
  пишут лишь швейные операции (крой закрывается при выпуске,
  ОТК/ВТО/упаковка — на своих гейтах), иначе «нет закрытия» означало
  бы «назад можно всегда»; шаг параллельной группы долгом быть не
  может — её AND-гейт не выпустит паспорт вперёд;
- **закрытый** шаг позади остаётся откатом (409) — повторный проход
  открывает либо ОТК (`OPERATION_REWORK_OPENED`), либо мастер;
- канал `complete` послабления не получает: там «операция позади» —
  это закрытие не той операции, на которую паспорт взят.

**Операцию ВЗЯТИЯ задаёт паспорт, а не смена** (26.08.2026,
`PassportsService.resolveOperationByPassportRoute`). `ShiftSession.
operationId` — свойство человека и станка: оно общее на все паспорта
на руках и переживает переход к другому заказу; маршрут — свойство
паспорта. Пока источником истины была смена, промах в списке операций
станка молча писал работу мимо маршрута (инцидент 15.08.2026, заказ
02-00001: станок ОВЕРЛОК привязан к двум операциям-двойникам,
`02 Ф ОВЕРЛОК` и `098642 ОВЕРЛОК`, в маршруте стоит вторая, смена
была открыта на первой). Правило:

- операция смены есть в маршруте (сама, через `OperationSubstitution`
  или наряд-допуск) — поведение прежнее;
- операции смены в маршруте нет, но оборудование смены умеет операцию
  ИЗ маршрута — паспорт встаёт на неё, без переоткрытия смены; выбор
  среди нескольких — первая по порядку маршрута, которую паспорт может
  взять сейчас; факт подстановки пишется в `AuditLog`
  (`PASSPORT_OPERATION_ROUTED`);
- оборудование не умеет ни одной маршрутной операции — 409
  `PASSPORT_SHIFT_OPERATION_NOT_ON_ROUTE` (вместо прежнего
  молчаливого пропуска по `offRouteWorkPolicy = WARN`).

Границы те же, что у `enforceOffRoutePolicy`: только `SEWING`, только
при `offRouteWorkPolicy ≠ OFF`, только для заказов со снимком
маршрута. Канал `complete` правило НЕ трогает — он закрывает то, НА
ЧТО паспорт был взят (см. инцидент 04.08.2026), иначе паспорта,
взятые до правки, стали бы незакрываемыми.
...
⚠️ **Это НЕ метка «операция на стороне» (§2.4a).** Три разные оси про
подряд, которые легко перепутать и нельзя «синхронизировать» походя:
`OrderOutsourceRequirement` — строка снимка ТЕХКАРТЫ (шелкография,
вышивка, печать этикеток) с лёгким операционным статусом и без сумм;
`OrderRouteStep.outsourced` — метка на ОПЕРАЦИИ маршрута, меняющая
план денег заказа; строки передачи и колонка «На стороне» в ERP —
объём и документ закупки на стороне ERP. Ни одна из них не считается
из другой.
...
**Удержание не может «съесть» начисления (аудит движка расчёта
13.09.2026, K1).** У строки с начислениями (`amountPieceworkRub +
amountSalaryRub > 0`) итог обязан остаться `amountToPayRub > 0`:
`PATCH` строки с корректировкой, дающей `≤ 0` (в т.ч. зачёт аванса
«в ноль»: 5 000 / −5 000), отвечает 422 `PAYROLL_ACCRUAL_LINE_NON_POSITIVE`
с подсказкой «удержать можно меньше N ₽»; та же проверка стоит в `pay`
...
по строке «к выплате ≤ 0» не создаётся, её начисления не закрываются
`PayrollPayoutLine` и следующая ведомость взяла бы их снова, а
удержание сгорело бы. Остаток удержания менеджер переносит в следующую
ведомость руками. Строка без начислений с одним удержанием
