...
- [x] **Шаг 3.** Seed-данные (размеры, операции, продукты, демо-сотрудники, оборудование, ячейки, базовые сдельные ставки `OperationRateBySize` / `Operation.fixedRate` — историческая таблица `PieceRate` удалена в PHASE 2 STEP 1, см. ADR-0020 §«PHASE 2 — drop legacy»)
...
- [x] **Operations management & unified pricing (post-Шаг 17).**
  ...
  менеджеров. Миграция бэкфилит данные из `PieceRate` (сама
  таблица удалена в PHASE 2 STEP 1, см. ADR-0020 §«PHASE 2 —
  drop legacy»). Существующий pipeline
