...
| GET   | `/api/shifts/my-unclosed`  | Any auth  | Паспорта с НЕЗАКРЫТОЙ операцией текущего сотрудника, которые уже уехали дальше по маршруту (секция «Не закрыто вами» на `/work`, поллинг 30 с). Правило — открытое назначение (последнее `ISSUED_TO_EMPLOYEE`/`OPERATION_SCAN` без `OPERATION_FINISHED`), только `SEWING`, только паспорта НЕ на руках (те показывает `current-work`). `IN_PROGRESS` **и `PACKED`** — долг переживает упаковку. Закрытие ЗАМЕСТИТЕЛЕМ (`OperationSubstitution` + наряд-допуски) засчитывается: иначе сплит-распошив (взяли 16/0001, закрыли 04) вечно висел бы ложным долгом. |
...
| POST  | `/api/orders/:id/reopen-calculation`                                       | SHOP_MANAGER (+ ADMIN)              | `CALCULATION_DONE → CALCULATION`. Body `ReopenOrderCalculationDto` (`{ reason? }`). Активный `OrderCostEstimate` помечает `REVOKED`, `Order.costEstimate*` обнуляет (включая `costEstimateStaleAt/Reason` — отметка про отозванную смету, аудит 13.09.2026 E1-12); `WorkshopNeed`/`PurchaseOrder`/`PurchaseReceipt` НЕ трогает. |
...
| POST  | `/api/orders/:id/operation-plan/recalculate`                               | SHOP_MANAGER (+ ADMIN)              | Ручной пересчёт snapshot-полей `Order.operationCostPlanRub` / `operationOutsourceCostPlanRub` / `operationTimePlanSec` / `operationPlanWarnings`. Запрещено в `CALCULATION_DONE` / `IN_PRODUCTION` / `DONE` / `CANCELLED` (`ORDER_OPERATION_PLAN_RECALCULATE_NOT_ALLOWED`). |
| PUT   | `/api/orders/:id/route-overrides`                                          | SHOP_MANAGER (+ ADMIN)              | Правка расценок / норм времени / способа оплаты **в рамках заказа** и метки сторонних услуг. Body `UpdateOrderRouteOverridesDto` (`packages/shared/src/routes.ts`): `{ steps: [{ stepId, pricingModeOverride?, rateOverride?, timeNormSecOverride?, outsourced?, outsourcePriceRub?, sizeOverrides?: [{ sizeId, rate, seconds, outsourcedQty }] }] }`. Пишет только снимок маршрута заказа (`OrderRouteStep` / `OrderRouteStepSizeOverride`) — справочник `Operation` и шаблон маршрута не трогаются. **Семантика полей:** не переданное поле = «не менять»; `null` = «сбросить на значение операции»; `sizeOverrides` — replace-all по шагу (строка считается непустой, если задан хотя бы один из `rate` / `seconds` / `outsourcedQty`). **Сторонние услуги** (ADR-0023, `docs/order-flow.md §5.1`): `outsourced` — операцию делает подрядчик, `outsourcePriceRub` — цена размещения за одно изделие (₽/шт), `outsourcedQty` — сколько штук размера отдано (целое ≥ 0, потолок формы `ROUTE_STEP_OUTSOURCED_QTY_MAX = 1 000 000`). Явный `outsourced: false` дополнительно обнуляет цену и ВСЕ `outsourcedQty` шага, чтобы выключенный подряд не всплыл при повторном включении метки. **Ошибки:** 404 `ORDER_NOT_FOUND`; 400 `ORDER_ROUTE_OVERRIDES_NOT_ALLOWED` (заказ `DONE` / `CANCELLED`); 400 `ORDER_ROUTE_STEP_NOT_FOUND` (шаг не принадлежит заказу); 400 `ORDER_ROUTE_OVERRIDE_SIZE_INVALID` (размер не входит в план); 400 `ORDER_ROUTE_OUTSOURCED_QTY_OVER_PLAN` (на сторону отдано больше, чем план по размеру — потолок это СУММА `OrderItem.qtyPlan` по всем строкам заказа с этим `sizeId`); 400 `ORDER_ROUTE_OVERRIDE_RATE_REQUIRED` (перевод на `FIXED` без расценки — **не** срабатывает, если шаг после правки уходит на сторону целиком: метка стоит, поразмерных объёмов нет). Side effects: пересчёт планового snapshot-а в той же транзакции (`recalculateAndWrite` в `DRAFT`/`CALCULATION`, иначе `recalculateAndWriteFromSnapshot` — в производстве источник истины маршрута снимок) + audit `ORDER_ROUTE_OVERRIDES_UPDATED`. Ответ — `OrderDetailDto`. |
...
`packages/shared/src/routes.ts` (переопределения шага заказа и
сторонние услуги). ADR: 0006, 0009, 0022, 0023.
...
| POST  | `/api/orders/:id/workshop-needs/calculate`        | ADMIN, SHOP_MANAGER | Body `CalculateWorkshopNeedsDto`. Пересчёт потребностей конкретного заказа. Гейт 409 `WORKSHOP_NEEDS_ALREADY_REVIEWED` (без `force`) считает только системные строки — ручные (`isManual`) пересчёт не трогает и они его не блокируют (аудит 13.09.2026, N2-2). Закупочный блок переносится на пересозданные строки: цена/валюта/поставщик/`packSize`/`comment`/`expectedDeliveryDate` (N2-7) и «К закупке» со сверкой (N2-4): `purchaseQty`, равное прежнему `calculatedQty`, следует за новым расчётом; своё число закупщика переносится, а при изменившемся расчёте в `warnings` и `calculationNote` строки попадает предупреждение с обоими числами. Режим `appendMissing` сверяет существующие строки по источнику (включая `CANCELLED`) и по описанию (живые строки; погашенные не мешают вернувшемуся материалу — N2-11), для схлопнутой строки «без цвета» — ещё и по парам расцветок `mergedFrom` и описанию без расцветки (N1-7/N2-3). |
...
Расчёт ЗАВЕРШАЮТ ПО КАЖДОМУ варианту: `OrderCostEstimate` принадлежит
варианту (`orderCalculationId`), смета активного варианта = «смета
заказа» (скоуп `ACTIVE_CALCULATION_ESTIMATE_WHERE`). Поэтому вкладки
живые до ЗАПУСКА В ПРОИЗВОДСТВО (`canSwitch` = true в
`DRAFT|CALCULATION|CALCULATION_DONE|SAMPLE_PRODUCTION`), а переключение
переносит snapshot-поля себестоимости заказа и двигает `Order.status`
между `CALCULATION` и `CALCULATION_DONE` — статус следует за активной
вкладкой.

| Метод | Путь                                              | RBAC                | Описание |
| ----- | ------------------------------------------------- | ------------------- | -------- |
| GET   | `/api/orders/:id/calculations`                    | Any auth            | Ряд вкладок (`OrderCalculationsDto`: `activeId`, `canSwitch`, items c `costTotalRub`-ярлыком, `sentToCalculationAt` и `costEstimateCompletedAt` — стадия ВАРИАНТА «черновик / отправлен / рассчитан`). Lazy-ensure: заказу без калькуляций заводится активная #0. 404 `ORDER_NOT_FOUND`. |
| POST  | `/api/orders/:id/calculations`                    | ADMIN, SHOP_MANAGER | «+ Вариант просчёта»: клон активного (старый получает снимок, новый активен, живые таблицы не меняются). Клон рождается ЧЕРНОВИКОМ (`sentToCalculationAt = null`) — его потребности считаются только явной кнопкой (см. `start-calculation`). Клон рождается и БЕЗ сметы: заказ снимает себестоимость прежней вкладки и возвращается в `CALCULATION`. Body `CreateOrderCalculationSchema` (`{ title? }`). 409 `ORDER_CALCULATION_LOCKED` после запуска в производство. Audit `ORDER_CALCULATION_CREATED`. |
| POST  | `/api/orders/:id/calculations/:calcId/activate`   | ADMIN, SHOP_MANAGER | Переключение активного варианта: capture текущего → restore снимка → пересборка производных (`resyncColorwayDerived` без пересчёта потребностей) + оверлей route-оверрайдов. Потребности живут per вариант (`WorkshopNeed.orderCalculationId`); переключение НИЧЕГО не считает (итерация 3 «стадия per вариант»): строки варианта только ре-линкуются к пересозданным расцветкам, а вариант-черновик рассчитывается явной кнопкой (`POST /orders/:id/start-calculation`, ветка isVariantCalc — статус заказа не меняется, вариант получает `sentToCalculationAt`, audit `ORDER_CALCULATION_VARIANT_SENT`). Себестоимость и статус расчёта переезжают на смету целевого варианта (есть завершённая смета → `CALCULATION_DONE`, нет → `CALCULATION`); после пересборки производных смета цели сверяется с её входами — расхождение ставит `Order.costEstimateStaleAt` + причину (аудит движка расчёта 13.09.2026, E1-2/V1-1), активация ничего не пересчитывает. В снимок входят также `routeCustomizedAt` + полный состав шагов (правленный холстом маршрут — свойство варианта, V1-2; старый снимок без поля маршрут заказа не трогает — ревью V1-2), `items` (тираж заказа без расцветок, V1-3), оверрайды шагов сопоставляются по вхождению операции (повтор ОТК/ВТО, V1-4). Гейты: 409 `ORDER_CALCULATION_LOCKED` (заказ в производстве), 409 `ORDER_CALCULATION_SNAPSHOT_INVALID`, 409 `ORDER_CALCULATION_PASSPORTS_COLOR_MISMATCH` (на стадии сигнального образца: расцветки под паспортами нет в целевом варианте), 400 `ORDER_ROUTE_OVERRIDE_RATE_REQUIRED` ДО мутаций (в снимке шаг `FIXED` без своей расценки, а у операции сняли `fixedRate`; шаг `BY_SIZE` без ставки по какому-то размеру плана цели ни в снимке, ни в `OperationRateBySize` — V1-5, зеркало гарда L1-10). Снимок цели стирается только по завершении всех фаз: повторный activate уже активного варианта — no-op, если снимка нет, иначе долечивает производные незавершённого переключения (V1-5). Audit `ORDER_CALCULATION_ACTIVATED`. |
...
| DELETE| `/api/orders/:id/calculations/:calcId`            | ADMIN, SHOP_MANAGER | Удалить НЕактивный вариант (его потребности удаляются, а завершённая смета переводится в `REVOKED` — осиротевшая строка иначе попала бы в скоуп активного варианта). 409 `ORDER_CALCULATION_ACTIVE_DELETE_FORBIDDEN` / `ORDER_CALCULATION_LAST_DELETE_FORBIDDEN` / `ORDER_CALCULATION_LOCKED`. Audit `ORDER_CALCULATION_DELETED`. |
...
| GET   | `/api/master-actions/passports/:id/self-operation-steps`              | SHOPFLOOR_MASTER, SHOP_MANAGER (+ ADMIN) | Read-only. Шаги снимка маршрута заказа с доступностью для взятия на себя (`available` + `blockedReason` — расчёт `PassportsService.previewOperationAvailability`, тот же гейт, что у «получить крой»), станками операции (`EquipmentOperation`), сдельной расценкой и суммой для этого паспорта (`pieceworkRate`/`pieceworkAmount` — `OperationsService.resolveRate` по размеру и снимку маршрута, `null` = сделка недоступна; незаполненная ставка тоже `null`, список не падает), `qty` (`qtyCut`) и предвыбором режима зачёта `defaultPayMode` (по типу оплаты актора). Возвращает `MasterSelfOperationStepsDto`. |
| POST  | `/api/master-actions/passports/:id/self-operation`                    | SHOPFLOOR_MASTER, SHOP_MANAGER (+ ADMIN) | Body `MasterSelfOperationDto` (`{ operationId, equipmentId?, payMode?, comment? }`; `reason` НЕ требуется — мастер фиксирует свою работу). Мастер выполнила операцию маршрута сама: техническая смена → `issueToEmployee` → `completeOperationByEmployee` → закрытие смены. `payMode` — как зачесть работу (14.09.2026): `PIECEWORK` — сдельная строка `OperationEntry` по расценке маршрута НЕЗАВИСИМО от `compensationType` мастера (окладнице тоже), `SALARY` — строки нет даже у сдельщицы; без поля — по типу оплаты, как раньше. `PIECEWORK` без расценки → 409 `MASTER_SELF_OPERATION_NO_PIECEWORK_RATE` ДО движения паспорта. Audit `MASTER_PASSPORT_SELF_OPERATION` (+ `payMode`, `pieceworkRate`, `pieceworkAmount`). Если завершение упало уже ПОСЛЕ выдачи (гонка, сбой БД), паспорт не остаётся «в работе у мастера»: компенсация снимает владельца (только его — движение по маршруту не откатывается), пишет audit `MASTER_PASSPORT_SELF_OPERATION_ROLLED_BACK` и пробрасывает исходную ошибку. |
...
Единственное, что мастер решает сама, — как зачесть работу (`payMode`).
Выбор доезжает до `EarningsService.createPendingForCompletedOperation`
через `completeOperationByEmployee(…, { pieceworkOverride: 'FORCE' |
'SKIP' })` и обходит `isPieceworkEligible` в обе стороны; расценку он не
обходит — окладная в заказе операция сделки не даст. Оклад
(`SalaryEntry`) действие по-прежнему не трогает ни в одном режиме.
...
| Метод | Путь | RBAC | Описание |
| ----- | ---- | ---- | -------- |
| GET   | `/api/master/employee-stats`                              | SHOPFLOOR_MASTER, SHOP_MANAGER (+ ADMIN) | Query `MasterEmployeeStatsQuerySchema` (`?from&to`, МОСКОВСКИЕ дни включительно). `MasterEmployeeStatsDto` — «кто сколько сделал» по `PassportEvent.OPERATION_FINISHED` + время в смене по `ShiftSegment` (`workedMinutes`, `ribbon` для одних суток). Строка есть и у того, кто был на смене, но ничего не закрыл. |
...
| GET   | `/api/master/employee-stats/day`                          | SHOPFLOOR_MASTER, SHOP_MANAGER (+ ADMIN) | Query `MasterEmployeeDayQuerySchema` (`?employeeId&from&to`, московские сутки включительно; день = `from === to`). `MasterEmployeeDayDto` — «табель»: отрезки смены (`ShiftSegment`), «где был» по участкам, присутствие/загрузка/паузы, выработка по операциям с `% нормы` (`Operation.timeNormSec`, только `timeNormMode = FIXED`) и `byDay` (часы по дням — график режимов «Неделя»/«Месяц»). События внутри отрезка (номера паспортов) отдаются ТОЛЬКО при `from === to`: за неделю их сотни. Отрезок через полночь обрезается границами суток. Периода без смен — нули, а не 404. |
...
| POST  | `/api/passports/:id/close-unclosed-operation` | Any auth                              | Тела нет. Закрывает СВОЮ незакрытую операцию по паспорту, который уже увели дальше (см. `GET /api/shifts/my-unclosed`). Пишет `OPERATION_FINISHED` + `OperationEntry` и НЕ двигает паспорт по маршруту (ни операцию, ни шаг, ни владельца, ни статус). Что закрывать, backend берёт из истории (открытое назначение этого сотрудника) — чужую работу приписать нельзя: 409 `PASSPORT_NO_UNCLOSED_WORK`. Тем же кодом отбивается операция, уже закрытая заместителем (иначе второе начисление за оплаченную работу). Работает и по `PACKED` — там начисление создаётся сразу `APPROVED` (второго `approvePendingForPassport` не будет). |
...
Аудит движка расчёта 13.09.2026 (E1-1, E1-10): `erpUnitPriceRub ≤ 0` (строка ЗП ERP без цены) —
«цена не задана»: хранится `null`, смета/план→факт/v2 берут `quotedPrice` закупщика цеха. Обе ручки
(`erp-link`, `erp-unlink`) заканчиваются `OrderCostEstimatesService.syncAfterNeedsChange` — смета и
`Order.costEstimateTotalRub` догоняют цену ERP (`domain.md §1.5`), а не при следующей соседней
правке. Ревью E1-1: ERP шлёт эти ручки ПОСТРОЧНО, поэтому путь `erpBatch` дебаунсит версии сметы —
активная смета моложе 60 с (`ERP_ESTIMATE_SYNC_WINDOW_MS`) новой версией не заменяется, заказ
получает `costEstimateStaleAt` + причину «Цена ERP обновлена…»; первая связь серии (и первая после
окна) пересчитывает и снимает отметку. Сметы неактивных вариантов на этом пути не пересобираются —
их расхождение ловит сверка при активации варианта.
...
| PATCH | `/api/payroll/accrual-documents/:id/lines/:lineId`              | SHOP_MANAGER, ADMIN | Только DRAFT. Body `UpdatePayrollAccrualDocumentLineDto` (`{ manualAdjustRub?, manualComment? }`). После изменения пересчитываются `amountToPayRub` и итоги документа. У строки с начислениями (`amountPieceworkRub + amountSalaryRub > 0`) итог обязан остаться `≥ 0` (полный зачёт «в ноль» допустим — ревью K1) — иначе 422 `PAYROLL_ACCRUAL_LINE_NON_POSITIVE` (K1). 404 `PAYROLL_ACCRUAL_DOCUMENT_LINE_NOT_FOUND`. AuditLog: `PAYROLL_ACCRUAL_DOCUMENT_LINE_UPDATED`. |
| POST  | `/api/payroll/accrual-documents/:id/pay`                        | SHOP_MANAGER, ADMIN | Только DRAFT. Строка «начисления есть, `amountToPayRub < 0`» блокирует проведение → 422 `PAYROLL_ACCRUAL_LINE_NON_POSITIVE`, документ остаётся DRAFT (K1). Документ переходит в `PAID`. Для каждой строки с начислениями (`amountToPayRub ≥ 0`) и для строки с одной положительной корректировкой создаётся `PayrollPayout` (статус `ISSUED`); полный зачёт «в ноль» (нетто 0) даёт выплату на 0 ₽ с PIECEWORK/SALARY-строками и `ADJUSTMENT = −Σ начислений` — начисления закрыты, заявка казначейства не создаётся (ревью K1). Строка без начислений с одним удержанием пропускается (её удержание в выплату не попадает; id — в `skippedLineIds` аудита). Перед созданием повторная проверка активной уникальности → 422 `PAYROLL_ACCRUAL_LINE_ALREADY_PAID`. **STEP 6.4:** если `manualAdjustRub ≠ 0`, создаётся дополнительная `PayrollPayoutLine` с `kind = ADJUSTMENT`, `operationEntryId = null`, `salaryEntryId = null`. AuditLog: `PAYROLL_ACCRUAL_DOCUMENT_PAID` (payload содержит `adjustmentsCount` / `totalAdjustRub` — только по реально созданным ADJUSTMENT-строкам — и `skippedLineIds`). |
...
- `PAYROLL_ACCRUAL_LINE_NON_POSITIVE` (422) — у строки с начислениями
  `amountToPayRub < 0` (удержание больше начислений). Отдаётся на `PATCH`
  строки и на `pay`. Аудит движка расчёта 13.09.2026, K1: раньше `pay`
  такую строку молча пропускал — выплата не создавалась, её начисления
  оставались «не выплаченными» и уходили в следующую ведомость повторно,
  удержание сгорало. Менеджер уменьшает корректировку так, чтобы к выплате
  осталось `≥ 0`, остаток переносит в следующую ведомость. Ревью K1: полный
  зачёт «в ноль» (нетто ровно 0) кодом НЕ отбивается — проводится выплатой
  на 0 ₽ (имя кода историческое). Строка без начислений с одним удержанием
  не задета (пропускается при `pay`, как раньше; удержание в выплату не
  попадает — см. `skippedLineIds`).
...
| PATCH | `/api/company-settings`    | SHOP_MANAGER, ADMIN | `UpdateCompanySettingsDto` (любое подмножество полей: legalName/shortName/INN/КПП/ОГРН/адреса/телефон/email/руководители/банк/БИК/р/с/к/с + `autoIssueMaterialsOnCutRelease?`, `allowNegativeMaterialStock?`, `sessionIdleTimeoutMinutes?`, `shiftAutoCloseTime?` (`"HH:MM"` по Москве или `null`), `shiftMaxDurationHours?` (`0` — без ограничения), `shiftAutoCloseMode?` (`LAST_ACTIVITY` / `AT_DEADLINE`)). Audit `COMPANY_SETTINGS_UPDATED`. |
...
### Автор действия из ERP и служебный сотрудник

Действие через машинный токен цех записывает от имени служебного сотрудника **«Интеграция ERP»**
(логин `erp-integration`, один на тенанта, создаётся лениво при первом запросе токена, PIN
случайный — войти им нельзя). Это правило владельца 02.09.2026 (`service/docs/kb/sewing.md`
§0.1): у полей «кто создал / кто принял / кто завершил расчёт» внешний ключ на `Employee`, и
синтетический id ронял бы запись на ключе. Таблица соответствия людей не заводится: пользователи
цеха переедут в ERP.

Кто из людей ERP нажал — заголовки `X-Sewing-Actor` (id пользователя ERP) и
`X-Sewing-Actor-Name` (имя, percent-encoded). `AuthGuard` кладёт их в контекст запроса
(`auth/actor-context.ts`), `AuditService.log` дописывает `actor: { id, name, source: 'erp' }` в
payload каждой строки журнала. Человеку с cookie заголовки не нужны и не читаются.

`X-Sewing-Tenant` — синоним `x-tenant-host` для сервер-серверного клиента (тенант резолвится
по нему, если web-заголовка нет).

Машинные скоупы разнесены по методам: класс — `<область>:read`, каждый POST/PATCH/PUT/DELETE —
`<область>:write`; там, где запись машине не положена при открытом на чтение классе
(`PATCH /salary/:id`, `POST /costs/snapshots/finalize`, `POST /print-jobs`, словарь характеристик),
стоит `@MachineClosed()`.
...
### Сдача заказов в ERP — очередь документов выпуска

Источник: `integrations/erp-production.controller.ts`, `erp-production.service.ts`. Единица —
ДОКУМЕНТ ВЫПУСКА по заказу (`ProductionDocument`, решение владельца 04.09.2026), а не паспорт.

| Метод | Путь | Доступ | Назначение |
|---|---|---|---|
| GET | `/api/integrations/erp-production?ready_from=&limit=` | `stock:read` | Сформированные (`READY`) документы выпуска по заказам из ERP, страница очереди. |
| PUT | `/api/integrations/erp-production` | `stock:write` | Журнал ответа ERP (`{ items: [{ order_id, state, erp_document_id, … }] }`) — след, не гейт. |

**Курсор (аудит движка расчёта 13.09.2026, G7-1).** У каждого документа в ответе есть
`queue_at` = max(`ready_at`, `recalculated_at`) — момент, когда он в последний раз стал
«новостью» для ERP. Страница отобрана по `queue_at >= ready_from` и отсортирована по
`queue_at ASC, id ASC`; ERP хранит курсор = max(`queue_at`) страницы и шлёт его следующим
`ready_from`. Ключ курсора и ключ сортировки — одно поле: раньше фильтр «`ready_at ≥ since` ИЛИ
`recalculated_at ≥ since`» при сортировке по `ready_at` давал курсору уезжать назад, как только
пересобранные документы заполняли страницу, и новые сдачи не приезжали никогда. Документ с
`queue_at`, равным курсору, приходит повторно (`>=`) — ERP гасит повтор по `document_number`,
он стабилен и переживает пересборку.

`ready_from` необязателен: без него отсечка берётся из настройки `erpFinishedGoodsSince`
(`fg_since`), без неё очередь ПУСТА — архив сдач не уезжает. `closed_from` — синоним
`ready_from` (старое имя). `limit` — 1…100, по умолчанию 20.

Перед выдачей документы страницы сверяются с фактами цеха по отпечатку (D1-3) — пересобранный
получает `recalculated_at = now` и уезжает в конец очереди; писатели поздних фактов (выдача и
возврат материала, утверждение начислений, прочие расходы, логистика, ответ ERP по списанию)
пересобирают сформированный документ сами, фоном (`ProductionDocumentsService.refreshLater`), и
он возвращается в очередь по `queue_at`. Изменение одних предупреждений себестоимости (ответ ERP
FAILED/EMPTY) — тоже пересборка (D1-12).
