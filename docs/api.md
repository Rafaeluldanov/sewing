...
| GET   | `/api/cutting-tasks/ready-for-release`               | CUTTER_ASSISTANT, SHOP_MANAGER, ADMIN | Доска помощника `/work/cut-orders`: заказы с `CuttingTask = DONE`, готовые к выпуску. `OrderReadyForReleaseDto[]` со `status` `NEW` (есть невыпущенные пары `(размер, рулон)`) / `DONE` (все выпущены) и счётчиком `releasedPairs/totalPairs`. |
| GET   | `/api/cutting-tasks/by-order/:orderId/release-state` | CUTTER_ASSISTANT, SHOP_MANAGER, ADMIN | Данные заказа для рулонного выпуска: размеры (с `perLayerQty` «на настиле»), рулоны и карта уже выпущенных пар `(размер, рулон)`. `OrderReleaseStateDto`. 404 `CUTTING_TASK_NOT_FOUND`. |
...
| PATCH | `/api/cutting-tasks/:id`                             | CUTTER, SHOP_MANAGER, ADMIN | Автосохранение прогресса: `SaveCuttingTaskProgressDto` (`perLayerQty` по размерам + полная замена набора рулонов). Требует `IN_PROGRESS`. |
| POST  | `/api/cutting-tasks/:id/complete`                    | CUTTER, SHOP_MANAGER, ADMIN | «Раскрой завершён»: финальное сохранение + `IN_PROGRESS → DONE` (`completedAt`). Паспорта не трогаются — выпуск дальше у помощника. |
...
| POST  | `/api/passports/release-from-rolls`           | CUTTER_ASSISTANT, SHOP_MANAGER, ADMIN | Рулонный выпуск помощником раскройщика. Body `ReleaseFromRollsDto` (`{ orderId, sizeId, cutDate, rollOrdinals[] }`). Количество и номер рулона берутся из завершённой задачи раскройщика (`CuttingTask`): на каждый рулон создаётся паспорт с `qtyCut = CuttingTaskRoll.layers × CuttingTaskSizeRow.perLayerQty` и `rollOrdinal = ordinal`. Сдельное за раскрой автоматически идёт `CuttingTask.assignedToId`. Идемпотентно: пары `(sizeId, ordinal)`, уже выпущенные (есть non-CANCELLED паспорт с этим `rollOrdinal`), пропускаются (`skipped`) — кейс «сломался принтер, продолжить с рулона». Возвращает `ReleaseFromRollsResultDto` (`{ created[], skipped[] }`). Сохраняет инварианты: заказ `IN_PRODUCTION`, closure-блок (ADR-0018), `Σ qtyCut ≤ остаток плана`. Ошибки: 400 `CUTTING_TASK_NOT_FOUND` / `CUTTING_TASK_NOT_DONE` / `CUTTING_ROLL_NOT_FOUND`, 400 `CUTTER_REQUIRED` (нет `assignedToId`), 422 `QTY_EXCEEDS_REMAINING_PLAN`, 409 `CUTTING_CLOSED`. |
