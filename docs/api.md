...
| POST  | `/api/cutting-tasks/:id/lays/:ordinal/reopen`        | CUTTER, SHOP_MANAGER, ADMIN | «Открыть расклад» — снять закрытие для правок настила. Разрешено, только пока по раскладу нет живых паспортов (409 `CUTTING_LAY_HAS_PASSPORTS`: сначала удалить их). Если задача была `DONE`, возвращается в `IN_PROGRESS`. Идемпотентно. Ошибки: 404 `CUTTING_LAY_NOT_FOUND`. |
