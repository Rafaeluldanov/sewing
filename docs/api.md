...
| DELETE| `/api/passports/:id`                          | SHOP_MANAGER (+ ADMIN)                | Управленческое удаление паспорта (откат ошибочного выпуска: неверный размер / рулон / кол-во). 204 No Content. В одной транзакции: чистит `PassportEvent`, `OperationEntry`, `PassportDefect`, удаляет сам `Passport` и пишет `AuditLog(PASSPORT_DELETED)`. `MaterialIssue.passportId` обнуляется автоматически (`onDelete: SetNull`). Блокеры: 409 `PASSPORT_HAS_BOX` (паспорт упакован в коробку), 409 `PASSPORT_HAS_APPROVED_EARNINGS` (есть подтверждённые сдельные начисления), 409 `PASSPORT_HAS_POSTED_MATERIAL_ISSUE` (привязан проведённый документ расхода). См. `docs/domain.md §7.8 «Удаление паспорта»`. |
