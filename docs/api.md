...
| GET   | `/api/master/employee-stats/day`                          | SHOPFLOOR_MASTER, SHOP_MANAGER (+ ADMIN) | Query `MasterEmployeeDayQuerySchema` (`?employeeId&date`, московские сутки). `MasterEmployeeDayDto` — «табель дня»: отрезки смены (`ShiftSegment`), «где был» по участкам, присутствие/загрузка/паузы и выработка по операциям с `% нормы` (`Operation.timeNormSec`, только `timeNormMode = FIXED`). Отрезок через полночь обрезается границами суток. Дня без смен — нули, а не 404. |
