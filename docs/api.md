...
`Order.costEstimateTotalRub` догоняют цену ERP сразу (`domain.md §1.5`), а не при следующей соседней
правке.
