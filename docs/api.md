...
| POST  | `/api/master-actions/passports/:id/self-operation`                    | SHOPFLOOR_MASTER, SHOP_MANAGER (+ ADMIN) | Body `MasterSelfOperationDto` (`{ operationId, equipmentId?, comment? }`; `reason` НЕ требуется — мастер фиксирует свою работу). Мастер выполнила операцию маршрута сама: техническая смена → `issueToEmployee` → `completeOperationByEmployee` → закрытие смены. Audit `MASTER_PASSPORT_SELF_OPERATION`. Если завершение упало уже ПОСЛЕ выдачи (гонка, сбой БД), паспорт не остаётся «в работе у мастера»: компенсация снимает владельца (только его — движение по маршруту не откатывается), пишет audit `MASTER_PASSPORT_SELF_OPERATION_ROLLED_BACK` и пробрасывает исходную ошибку. |
