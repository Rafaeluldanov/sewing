...
- `.display-board { display: grid; grid-template-columns:
  minmax(0, 2fr) minmax(calc(var(--display-u) * 24), 1fr);
  min-height: 0; min-width: 0; }` — матрица слева
  (растягивается), оборудование справа (минимум — 24 единицы
  шкалы, см. §9.5, до 1fr).
...
| `desktop` | 1200–1599px | базовая шкала (§9.5), без layout-правок |
| `tv` | `min-width: 1600px` | поднятая единица шкалы (§9.5), board остаётся 2-колоночным (§9.2 выше). Отдельного 4K-слоя больше нет: на ≥2400px шкала упирается в потолок `clamp`'а |
| `portrait-kiosk` | `orientation: portrait` **и** `min-height: 1400px` | TV, повешенный вертикально (1080×1920, 2160×3840): киоск без внешнего скролла, board стопкой `1.5fr / 1fr`, шкала считается от ШИРИНЫ (§9.5) |
...
- **киоск** (`desktop`, `tv`, `portrait-kiosk`) — экран
...
### 9.5 Единая шкала `--display-u` и авто-подгонка матрицы

**Проблема.** Размеры витрины жили тремя независимыми способами:
`clamp(rem, vw, rem)` в базовом слое, ФИКСИРОВАННЫЕ `rem` в
TV-слоях (≥1600 / ≥2400) и ещё раз фиксированные — в портретном
киоске. Ни один не учитывал ВЫСОТУ вьюпорта, поэтому любой экран
шире 1600px — что TV на стене, что окно браузера на 27" мониторе —
получал одинаковые «телевизорные» размеры (часы 4rem, ячейка
1.7rem, KPI 2.8rem): шапка с KPI съедали пол-экрана, а матрица
всегда упиралась в собственный скролл.

**Решение — одна единица.** `.display-screen` задаёт

```css
--display-u-base: clamp(0.58rem, min(0.72vw, 1.15vh), 1.5rem);
--display-fit: 1;
--display-u: var(--display-u-base);
```

и ВСЕ размеры витрины (шрифты, padding'и, gap'ы, ширины колонок,
`top` второй строки шапки) выражены как `calc(var(--display-u) * N)`.
`min(vw, vh)` — потому что матрица растёт в обе стороны: вширь от
числа операций маршрута, ввысь от «цвет × размер».

Ориентиры: 1440×900 → ≈10.4px, 1920×1080 → ≈13.4px (TV-слой),
2560×1440 → ≈17.9px, 4K → 24px (потолок clamp'а). Это примерно на
четверть меньше прежнего десктопного масштаба.

Слои-исключения правят ОДНУ строку — `--display-u-base`, а не
двадцать `font-size`:

| Слой | Формула | Зачем |
|---|---|---|
| `tv` (≥1600) | `clamp(0.7rem, min(0.78vw, 1.24vh), 1.5rem)` | читаемость с 3-7 м |
| `portrait-kiosk` | `clamp(0.7rem, min(1.15vw, 0.62vh), 1.5rem)` | оси местами: на 1080×1920 ведущая — ширина, иначе единица вырождается |

Ручные слои (`phone`, `compact`, низкий экран) сохраняют свои
подобранные под руку размеры — они идут ПОЗЖЕ в каскаде и
перебивают шкалу.

**Авто-подгонка (`--display-fit`).** CSS-шкала не знает, сколько в
конкретном заказе цветов, размеров и операций: 3×5 с маршрутом из
четырёх операций влезают на любой экран, 8×7 с десятью — нет.
Поэтому `useMatrixFit` (`display-board.tsx`) сравнивает
`scrollHeight/Width` таблицы с `clientHeight/Width` её scroll-зоны
и ужимает шкалу ровно во столько раз, во сколько не влезает.
Значение уходит инлайновой переменной на саму `<table>`, где её
подхватывает `.display-matrix { --display-u: calc(var(--display-u-base)
* var(--display-fit)) }`.

Три вещи, без которых это разъезжается:

1. **Подгоняется только `<table>`, не блок целиком.** Заголовок
   блока остаётся на шкале экрана, значит высота scroll-зоны от
   подгонки не зависит и итерации сходятся. Иначе: шрифт ↓ →
   заголовок ↓ → места больше → шрифт ↑ → дребезг.
2. **Только киоск-режим.** В ручных слоях страница скроллится
   намеренно, а размеры подобраны под руку. Признак режима
   читается из САМОГО CSS (`overflow-y` витрины: `hidden` = киоск),
   а не повторением брейкпоинтов в JS — иначе два списка молча
   разъедутся при следующей правке `globals.css`.
3. **Гистерезис и пол.** Растём обратно только при зазоре > 5%
   (иначе экран «дышит» шрифтом на каждом polling-тике), ниже
   `MATRIX_FIT_MIN = 0.55` не ужимаемся — там матрица честно
   оставляет себе внутренний скролл.
