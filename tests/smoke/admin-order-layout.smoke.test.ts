...
describe('/admin/orders/new — мастер создания заказа', () => {
  ...
  test('шаг проверки предупреждает о гейтах ДО клика', () => {
    ...
    // (ORDER_ITEMS_REQUIRED). Мастер считает то же, что backend, и
    // говорит об этом на шаге «Проверка», а не ошибкой после нажатия.
    // Этап «техкарты → номенклатура»: материалы даёт спецификация
    // лекала, отдельного предупреждения о техкарте больше нет.
