 ...
 *   5. Сдвоенная шапка split-режима покрывает все 10 колонок тела
 *      (2 + 4 + 1 + 2 + 1): при 3 колонках зоны расхода заголовки съезжали
 *      влево, и «Закупка» вставала над служебной стрелкой.
 */
...
describe('спецификация расцветки — заведение материалов пачкой', () => {
  ...
  test('сдвоенная шапка покрывает все колонки тела', () => {
    const src = readSrc(
      'apps/web/components/orders/colorways/colorway-spec.tsx',
    );
    const head = src.slice(
      src.indexOf('{hasSplitLines && ('),
